...
| [`opc-diameter-transport`](crates/opc-diameter-transport/) | Bounded Diameter TLS/TCP and RFC 6083 DTLS/SCTP connectors and acceptors: direct and consuming in-band CER/CEA flows, exact SPIFFE authentication, coherent credential rotation, kernel SCTP-AUTH exporter-key transitions, bounded full-duplex application/control runtime, exact DWR/DWA and DPR/DPA correlation, and RFC 6733 simultaneous-open election *(realm routing, reconnect policy, external IPsec attestation, and application state remain product-owned)*. | [Crate boundary](crates/opc-diameter-transport/README.md) |
...
| [`opc-proto-ngap`](crates/opc-proto-ngap/) | Experimental NGAP APER: outcome-aware framing, canonical containers and bounded N3IWF ID/NAS/key/location field codecs, 15-outcome Release 18 evidence, and raw-preserving re-encode. | [RFC 005](docs/rfc/005-protocol-framework.md), [ADR 0013](docs/adr/0013-ngap-asn1-strategy.md) |
...
| [`opc-sa-mirror`](crates/opc-sa-mirror/) | Live IPsec SA keymat mirroring for near-hitless failover in which keys never persist: producer/sink/takeover ports, in-memory zeroizing standby custody, an mTLS-only keymat transport, and pre-validated resume input that ESP consumers pair with applied XFRM counter proof before fenced re-pin *(experimental)*. | [RFC 015](docs/rfc/015-live-sa-mirror.md), [ADR 0018](docs/adr/0018-epc-untrusted-access-sdk-boundary.md) |
| [`opc-route-steering`](crates/opc-route-steering/) | Safe Linux route/rule steering model with singleton convergence; scoped exclusive-writer owned-collection reconciliation bounded to 50,000-route/50,000-rule final and desired sets and a 100,000-route/100,000-rule install-before-delete transient set; a mock backend; and redaction-safe errors for packet-core CNFs. | [ADR 0018](docs/adr/0018-epc-untrusted-access-sdk-boundary.md), [RFC 011](docs/rfc/011-node-dataplane-resource-contract.md) |
