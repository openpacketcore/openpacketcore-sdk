...
| [`opc-diameter-transport`](crates/opc-diameter-transport/) | Experimental bounded Diameter TLS/TCP connector, acceptor, exact stream framing, role-bound direct and consuming in-band CER/CEA flows, mutual SPIFFE authentication, synchronous credential-epoch retirement, bounded full-duplex application/control runtime, exact DWR/DWA and DPR/DPA correlation, and RFC 6733 simultaneous-open election *(TLS/TCP only; no realm routing, reconnect policy, DTLS/SCTP, or product-readiness claim)*. | [Crate boundary](crates/opc-diameter-transport/README.md) |
