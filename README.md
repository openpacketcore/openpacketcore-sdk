...
| [`opc-ipsec-lb`](crates/opc-ipsec-lb/) | Pure SWu IKE/IPsec load-balancing primitives: tagged SPI policy, classifier, rendezvous selector, cookie helper, failover safety guards, audited ownership-fenced re-pin coordination, session-store ownership source, Host-XDP steering backend, BGP route-export VIP advertiser, and backend ports. | [ADR 0018](docs/adr/0018-epc-untrusted-access-sdk-boundary.md), [RFC 011](docs/rfc/011-node-dataplane-resource-contract.md) |
