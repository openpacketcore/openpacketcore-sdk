...
fn dtls12_resends_each_flight_epoch_and_sequence_increase() {
    ...
    let config_client = Arc::new(Config::builder().build().expect("valid default config"));
    let config_server = Arc::new(Config::builder().build().expect("valid default config"));
...
fn dtls12_duplicate_triggers_server_resend_of_final_flight() {
    let result = prepare_server_final_flight_resend(
        Config::builder()
            .build()
