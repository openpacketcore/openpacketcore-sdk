...
`PeerCertChain` for multiple certificates. A singleton event remains queued if
the poll buffer is too small, so retry after `BufferTooSmall`. The crate uses
the leaf certificate to verify DTLS handshake messages, but it does not perform
PKI validation. Your application must handle both variants and validate the
peer according to its policy (chain building, name/EKU checks, pinning,
revocation, etc.).
