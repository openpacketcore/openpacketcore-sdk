...
pub enum Error {
    ...
    TransmitQueueFull,
    /// One encoded record cannot fit within the configured path MTU.
    RecordExceedsPathMtu {
        /// Encoded record bytes, including record and cipher overhead.
        record_bytes: usize,
        /// Configured path MTU.
        path_mtu: usize,
    },
...
pub enum UnexpectedMessageError {
    /// A parsed handshake body did not match its authenticated message type.
    HandshakeBodyTypeMismatch,
...
pub enum InvalidStateError {
    /// The endpoint's internal role/version state is temporarily unavailable.
    EndpointUnavailable,
    /// No cipher suite has been selected for the connection.
    NoCipherSuiteSelected,
    /// A receive-queue path expected a handshake record.
    ExpectedHandshakeRecord,
    /// An outbound record was marked appendable but no datagram existed.
    OutgoingQueueMissing,
...
pub enum CryptoError {
    ...
    /// The configured DTLS 1.2 protected-record encryption limit was reached.
    AeadEncryptionLimitReached {
        /// The configured maximum number of protected-record encryptions.
        limit: u64,
    },
    ...
    /// A configured certificate chain cannot be represented on the wire.
...
pub enum SecurityError {
    ...
    /// A server sent an extension the client did not offer.
    UnsolicitedServerExtension {
        /// The unexpected TLS extension type.
        extension_type: u16,
    },
    /// A ClientHello signature_algorithms extension was malformed.
    MalformedClientSignatureAlgorithms,
    /// No offered signature/hash pair can be executed by the local key.
    NoMutuallyAcceptableSignatureAlgorithm,
    /// The server omitted CertificateRequest while the client required mutual
    /// certificate authentication.
    ServerDidNotRequestClientCertificate,
...
pub enum ConfigError {
    /// No cryptographic provider was configured or compiled in.
    NoCryptoProvider,
    ...
    /// The configured AEAD encryption limit exceeds the DTLS sequence space.
    AeadEncryptionLimitTooLarge {
        /// The configured encryption limit.
        limit: u64,
        /// The maximum executable encryption limit.
        maximum: u64,
    },
    /// A zero handshake timeout cannot permit any handshake to execute.
    HandshakeTimeoutZero,
    ...
    /// A client-certificate request is required, but PSK authentication is
    /// configured and cannot carry certificates.
    ServerCertificateRequestRequiredWithPsk,
    ...
    /// The classic RFC 6083 key transition is incompatible with the DTLS 1.3
    /// exporter schedule.
...
pub enum CryptoProviderValidationError {
    ...
    /// A DTLS 1.2 suite reported parameters inconsistent with its identifier.
    Dtls12CipherSuiteParametersInvalid(Dtls12CipherSuite),
    /// No DTLS 1.2 AEAD test vector exists for the suite.
    NoDtls12AeadTestVector(Dtls12CipherSuite),
    /// A DTLS 1.2 AEAD validation vector's additional data exceeds the record bound.
    Dtls12AeadAdditionalDataTooLong(Dtls12CipherSuite),
    /// Creating a DTLS 1.2 AEAD cipher failed.
    Dtls12AeadCreateFailed {
        /// The DTLS 1.2 cipher suite under validation.
        suite: Dtls12CipherSuite,
        /// The underlying crypto failure.
        source: CryptoError,
    },
    /// DTLS 1.2 AEAD encryption failed.
    Dtls12AeadEncryptFailed {
        /// The DTLS 1.2 cipher suite under validation.
        suite: Dtls12CipherSuite,
        /// The underlying crypto failure.
        source: CryptoError,
    },
    /// DTLS 1.2 AEAD encryption returned the wrong output.
    Dtls12AeadEncryptWrongOutput(Dtls12CipherSuite),
    /// DTLS 1.2 AEAD decryption failed.
    Dtls12AeadDecryptFailed {
        /// The DTLS 1.2 cipher suite under validation.
        suite: Dtls12CipherSuite,
        /// The underlying crypto failure.
        source: CryptoError,
    },
    /// DTLS 1.2 AEAD decryption returned the wrong output.
    Dtls12AeadDecryptWrongOutput(Dtls12CipherSuite),
    ...
    /// An AEAD validation vector's additional data exceeds the record bound.
    AeadAdditionalDataTooLong(Dtls13CipherSuite),
...
impl InternalError {
    ...
    pub(crate) fn handshake_body_type_mismatch() -> Self {
        Error::UnexpectedMessage(UnexpectedMessageError::HandshakeBodyTypeMismatch).into()
    }
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::RecordExceedsPathMtu {
                record_bytes,
                path_mtu,
            } => write!(
                f,
                "encoded DTLS record length {record_bytes} exceeds path MTU {path_mtu}"
            ),
...
impl fmt::Display for UnexpectedMessageError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::HandshakeBodyTypeMismatch => {
                write!(f, "handshake body type mismatch")
            }
...
impl fmt::Display for InvalidStateError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::EndpointUnavailable => write!(f, "endpoint state unavailable"),
            Self::NoCipherSuiteSelected => write!(f, "no cipher suite selected"),
            Self::ExpectedHandshakeRecord => write!(f, "expected handshake record"),
            Self::OutgoingQueueMissing => write!(f, "outgoing queue missing"),
...
impl fmt::Display for CryptoError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::AeadEncryptionLimitReached { limit } => {
                write!(f, "DTLS 1.2 AEAD encryption limit reached ({limit})")
            }
            ...
            Self::CertificateChainTooLarge => write!(f, "certificate chain too large"),
...
impl fmt::Display for SecurityError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsolicitedServerExtension { extension_type } => {
                write!(f, "server sent unsolicited extension type {extension_type}")
            }
            Self::MalformedClientSignatureAlgorithms => {
                write!(f, "client signature_algorithms extension is malformed")
            }
            Self::NoMutuallyAcceptableSignatureAlgorithm => {
                write!(f, "no mutually acceptable signature algorithm")
            }
            Self::ServerDidNotRequestClientCertificate => {
                write!(f, "server did not request a client certificate")
            }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NoCryptoProvider => write!(
                f,
                "no crypto provider is available; configure one or enable a provider feature"
            ),
            ...
            Self::AeadEncryptionLimitTooLarge { limit, maximum } => write!(
                f,
                "aead_encryption_limit {limit} exceeds the DTLS maximum {maximum}"
            ),
            Self::HandshakeTimeoutZero => {
                write!(f, "handshake timeout must be greater than zero")
            }
            ...
            Self::ServerCertificateRequestRequiredWithPsk => write!(
                f,
                "requiring a server CertificateRequest is incompatible with PSK authentication"
            ),
            ...
            Self::Rfc6083Dtls13Unsupported => write!(
                f,
                "classic RFC 6083 mode supports DTLS 1.2 only; DTLS 1.3 requires a different SCTP-AUTH profile"
            ),
            Self::Rfc6083DeterministicRngUnsupported => write!(
                f,
...
impl fmt::Display for CryptoProviderValidationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Dtls12CipherSuiteParametersInvalid(suite) => {
                write!(f, "DTLS 1.2 cipher suite parameters are invalid: {suite:?}")
            }
            Self::NoDtls12AeadTestVector(suite) => {
                write!(f, "no DTLS 1.2 AEAD validation vector for {suite:?}")
            }
            Self::Dtls12AeadAdditionalDataTooLong(suite) => {
                write!(f, "DTLS 1.2 AEAD validation AAD is too long for {suite:?}")
            }
            Self::Dtls12AeadCreateFailed { suite, source } => {
                write!(f, "DTLS 1.2 AEAD creation failed for {suite:?}: {source}")
            }
            Self::Dtls12AeadEncryptFailed { suite, source } => {
                write!(f, "DTLS 1.2 AEAD encryption failed for {suite:?}: {source}")
            }
            Self::Dtls12AeadEncryptWrongOutput(suite) => {
                write!(f, "DTLS 1.2 AEAD encryption was incorrect for {suite:?}")
            }
            Self::Dtls12AeadDecryptFailed { suite, source } => {
                write!(f, "DTLS 1.2 AEAD decryption failed for {suite:?}: {source}")
            }
            Self::Dtls12AeadDecryptWrongOutput(suite) => {
                write!(f, "DTLS 1.2 AEAD decryption was incorrect for {suite:?}")
            }
            ...
            Self::AeadAdditionalDataTooLong(suite) => {
                write!(
                    f,
                    "AEAD additional data is too long for DTLS 1.3 suite {suite:?}"
                )
            }
