...
//! - **AEAD**: AES‑GCM 128/256, ChaCha20‑Poly1305, and AES‑128‑CCM‑8
//!   (no CBC/EtM modes).
...
//!
//! ## Certificate model
//! During the handshake the engine emits either [`Output::PeerCert`][peer_cert]
//! with a borrowed leaf certificate or
//! [`Output::PeerCertChain`][peer_cert_chain] with an owned chain in leaf-to-root
//! wire order. Both protocol versions retain `PeerCert` for a singleton and use
//! `PeerCertChain` for multiple certificates. A singleton event is retryable
//! with a larger poll buffer when [`Output::BufferTooSmall`] is returned. The
//! crate uses the leaf certificate to verify DTLS handshake messages, but it
//! does not perform PKI validation. Your application must handle both variants
//! and validate the peer according to its policy (chain building, name/EKU
//! checks, pinning, revocation, etc.).
//!
//! For mutual certificate authentication, servers enable
//! `ConfigBuilder::require_client_certificate(true)` and clients enable
//! `ConfigBuilder::require_server_certificate_request(true)`. The latter is
//! default-off for compatibility and fails the handshake if the server omits
//! `CertificateRequest`; it cannot be combined with PSK authentication.
//!
//! ## RFC 6083 DTLS over SCTP
//!
//! `ConfigBuilder::rfc6083_sctp()` selects a DTLS 1.2-only profile for a reliable,
//! message-oriented SCTP embedding. It emits exactly one DTLS record per
//! `Output::Packet`, suppresses DTLS retransmission/replay-window machinery, and
//! uses the `EXPORTER_DTLS_OVER_SCTP` exporter plus explicit sender-dry/SCTP-AUTH
//! transition events. This profile never advertises, selects, or exports the
//! RFC 5764 `use_srtp` extension; an unsolicited server `use_srtp` is rejected.
//! The embedding must complete each `Rfc6083Prepare*` operation before polling
//! again.
//!
...
//! - `PeerCertChain(Vec<Vec<u8>>)`: owned leaf-to-root chain — validate in your app
...
//!                 }
//!                 Output::PeerCertChain(_chain) => {
//!                     // Validate the complete leaf-to-root peer chain
//!                 }
...
//! [peer_cert_chain]: https://docs.rs/dimpl/latest/dimpl/enum.Output.html#variant.PeerCertChain
...
pub struct DtlsCertificate {
    ...
    /// Intermediate certificates in leaf-to-root order.
    ///
    /// Trust anchors SHOULD be omitted.
    pub intermediates: Vec<Vec<u8>>,
    /// Private key in DER format.
    pub private_key: Vec<u8>,
}

impl Clone for DtlsCertificate {
    fn clone(&self) -> Self {
        Self {
...
impl Dtls {
    ...
    ///
    /// If `config` has no PSK configured, construction remains infallible but
    /// the handshake fails closed when it cannot resolve PSK credentials.
    ...
    /// Return the selected DTLS 1.2 cipher suite once negotiation completes.
    pub fn dtls12_cipher_suite(&self) -> Option<crypto::Dtls12CipherSuite> {
        match self.inner.as_ref()? {
        ...
        match (self.is_active(), active) {
            (true, false) => {
                ...
                match inner {
                    ...
                    Inner::ClientPending(client) => {
                        // The auto-sense version is not known yet, so there is
                        // no server version to convert to. Preserve the client.
                        self.inner = Some(Inner::ClientPending(client));
                    }
                    other => self.inner = Some(other),
                }
            }
            (false, true) => {
                let Some(inner) = self.inner.take() else {
                ...
                match inner {
                    ...
                    Inner::Server13(s) => {
                        if s.is_auto_mode() {
                            let hybrid = match auto::HybridClientHello::new(s.config()) {
                                Ok(hybrid) => hybrid,
                                Err(_) => {
                                    self.inner = Some(Inner::Server13(s));
                                    return;
                                }
                            };
                            let (config, certificate, now, _) = s.into_parts();
                            let cp =
                                ClientPending::new_with_hybrid(hybrid, config, certificate, now);
    ...
    pub fn handle_packet(&mut self, packet: &[u8]) -> Result<(), Error> {
        ...
        if self
            .inner
            .as_ref()
            .ok_or(Error::InvalidState(InvalidStateError::EndpointUnavailable))?
            .is_pending()
        {
    ...
    fn handle_pending_auto(&mut self, packet: &[u8]) -> Result<(), Error> {
            ...
            .ok_or(Error::InvalidState(InvalidStateError::EndpointUnavailable))?
        {
            ...
            _ => Err(Error::InvalidState(InvalidStateError::EndpointUnavailable)),
        }
    }
    ...
    fn handle_pending_auto_client(&mut self, packet: &[u8]) -> Result<(), Error> {
        ...
        let inner = self
            ...
            .ok_or(Error::InvalidState(InvalidStateError::EndpointUnavailable))?;
        let cp = match inner {
            Inner::ClientPending(cp) => cp,
            other => {
                self.inner = Some(other);
                return Err(Error::InvalidState(InvalidStateError::EndpointUnavailable));
            }
        };
        ...
        match version {
            ...
            auto::DetectedVersion::Unknown => Err(Error::UnexpectedMessage(
                crate::UnexpectedMessageError::UnrecognizedAutoServerResponse,
            )),
        }
    }
...
pub enum Output<'a> {
    ...
    /// name/EKU checks, pinning, etc.). Both DTLS versions use this borrowed
    /// variant for a singleton certificate. If the provided poll buffer is too
    /// small, [`Self::BufferTooSmall`] is returned and the event remains queued.
    PeerCert(&'a [u8]),
    /// Owned peer certificate chain in leaf-to-root wire order.
    ///
    /// The application is responsible for chain building and all trust,
    /// identity, EKU, revocation, and pinning policy. Both DTLS versions use
    /// this variant when the wire chain contains multiple certificates.
    PeerCertChain(Vec<Vec<u8>>),
    /// Extracted DTLS-SRTP keying material and selected SRTP profile.
    KeyingMaterial(KeyingMaterial, SrtpProfile),
    /// RFC 6083 endpoint-pair shared secret.
    ///
    /// In RFC 6083 mode this event is emitted before the corresponding
    /// first record protected with the next SCTP-AUTH key can be polled. The
    /// SCTP embedding must install the secret inactive before polling again.
    Rfc6083KeyingMaterial(KeyingMaterial),
    /// RFC 6083 sender-drain barrier before ChangeCipherSpec.
    ///
    /// The embedding must prove every preceding user message sender-dry while
    /// keeping the current SCTP-AUTH key active. ChangeCipherSpec is emitted
    /// under that old key after this barrier.
    Rfc6083PrepareChangeCipherSpec,
    /// RFC 6083 sender-drain and SCTP-AUTH activation barrier before Finished.
    ///
    /// The embedding must prove all earlier user messages sender-dry and
    /// activate the previously emitted key before polling the engine again.
    Rfc6083PrepareEpoch,
    /// RFC 6083 sender-drain barrier before a reciprocal close_notify.
    ///
    /// The embedding must prove all earlier user messages sender-dry before
    /// polling the engine for the queued alert.
    Rfc6083PrepareCloseNotify,
...
impl fmt::Debug for Output<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Rfc6083KeyingMaterial(v) => {
                write!(f, "Rfc6083KeyingMaterial({})", v.len())
            }
            Self::Rfc6083PrepareChangeCipherSpec => {
                write!(f, "Rfc6083PrepareChangeCipherSpec")
            }
            Self::Rfc6083PrepareEpoch => write!(f, "Rfc6083PrepareEpoch"),
            Self::Rfc6083PrepareCloseNotify => write!(f, "Rfc6083PrepareCloseNotify"),
...
mod test {
    ...
    #[test]
    fn new_12_psk_without_psk_config_is_infallible() {
        let config = Arc::new(Config::builder().build().expect("valid default config"));
        let dtls = Dtls::new_12_psk(config, Instant::now());
        assert_eq!(dtls.protocol_version(), Some(ProtocolVersion::DTLS1_2));
    }

    #[test]
    fn new_12_with_empty_certificate_is_infallible() {
        let cert = generate_self_signed_certificate().expect("Failed to generate cert");
        let config = Arc::new(Config::builder().build().expect("valid default config"));
        ...
        let dtls = Dtls::new_12(config, empty, Instant::now());
        assert_eq!(dtls.protocol_version(), Some(ProtocolVersion::DTLS1_2));
    }
    ...
    fn dtls12_server_accepts_unknown_ec_point_formats_extension() {
        ...
        extensions.extend_from_slice(&[0x00, 0x04]); // extension body length
        extensions.extend_from_slice(&[0x00, 0x02]); // algorithm vector length
