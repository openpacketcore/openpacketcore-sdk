...
/// RFC 6083 section 3.2 DTLS path MTU.
pub const RFC6083_DTLS_MTU: usize = 16_384;

/// Minimum per-direction datagram queue capacity for the RFC 6083 profile.
///
/// A maximum-size admitted DTLS 1.2 certificate chain can require 17
/// single-record datagrams, and a complete certificate handshake flight can
/// require 21. Keeping 32 entries in each direction leaves bounded headroom
/// for the remaining records in the flight while preserving RFC 6083's
/// one-record-per-SCTP-message invariant.
const RFC6083_MIN_QUEUE_DATAGRAMS: usize = 32;
const MAX_AEAD_ENCRYPTIONS: u64 = 1_u64 << 48;
...
///
/// Build with [`Config::builder()`]. Construction is fallible because the
/// selected cryptographic provider is validated before use.
#[derive(Clone)]
pub struct Config {
    mtu: usize,
    max_queue_rx: usize,
    ...
    require_server_certificate_request: bool,
...
impl Config {
    ...
    pub fn builder() -> ConfigBuilder {
        ConfigBuilder {
            ...
            require_server_certificate_request: false,
    ...
    /// For a certificate-authenticated client, require the server to request
    /// the client's certificate.
    ///
    /// This provides a fail-closed mutual-authentication contract: a server
    /// that omits `CertificateRequest` is rejected before the client sends its
    /// key exchange or reports a connected session. It is disabled by default
    /// for compatibility with server-authenticated-only deployments.
    ///
    /// This policy is incompatible with RFC 4279 PSK authentication, where
    /// neither endpoint sends a certificate.
    #[inline(always)]
    pub fn require_server_certificate_request(&self) -> bool {
    ...
    /// Whether DTLS record-layer replay detection is enabled.
    ///
    /// This is enabled by default for datagram transports. RFC 6083 requires
    /// it to be disabled when DTLS runs over reliable SCTP.
    #[inline(always)]
    pub fn replay_detection(&self) -> bool {
        self.replay_detection
    }

    /// Whether the RFC 6083 DTLS-over-SCTP profile is enabled.
    #[inline(always)]
    ...
    ///
    /// When set, test-visible hello randoms, cookie secrets, retry jitter, and
    /// DTLS 1.3 key-update threshold jitter are deterministic. Record AEAD
    /// nonces are always derived from the write IV and record sequence number,
    /// never from this RNG. This is useful for testing and reproducibility.
    /// RFC 6083 configurations reject a seed because repeated hello/cookie
    /// values can repeat derived SCTP-AUTH material.
    ///
    ...
    /// Maximum number of protected-record AEAD encryptions per write key.
    ///
    /// DTLS 1.2 fails further sends at the limit because it has no KeyUpdate.
    /// DTLS 1.3 uses the limit to schedule a KeyUpdate. Defaults to 2^23
    /// (8,388,608).
    #[inline(always)]
...
impl ConfigBuilder {
    ...
    /// Set whether a certificate-authenticated client requires the server to
    /// send `CertificateRequest`.
    ///
    /// When enabled, omission of `CertificateRequest` is a terminal typed
    /// security error and the client does not report a connected session.
    /// Defaults to false.
    ///
    /// This cannot be combined with
    /// [`with_psk_client`](Self::with_psk_client) or
    /// [`with_psk_server`](Self::with_psk_server), because RFC 4279 PSK
    /// handshakes do not use certificates.
    pub fn require_server_certificate_request(mut self, require: bool) -> Self {
        self.require_server_certificate_request = require;
        self
    }
    ...
    /// Enable or disable DTLS record-layer replay detection.
    ///
    /// Disabling this is appropriate only when an authenticated reliable
    /// transport provides ordering and duplicate suppression, as RFC 6083
    /// requires for DTLS over SCTP.
    pub fn replay_detection(mut self, enabled: bool) -> Self {
        self.replay_detection = enabled;
        self
    }

    /// Configure the mandatory DTLS engine controls for RFC 6083.
    ///
    /// This disables record replay detection and DTLS flight
    /// retransmissions, and treats the SCTP record budget as the path MTU.
    /// The engine also emits an exporter event before each Finished record so
    /// the embedding SCTP transport can switch its SCTP-AUTH key first.
    ///
    /// RFC 6083 requires one DTLS record per SCTP message. At build time the
    /// profile therefore raises both bounded datagram queues to at least 32,
    /// enough for every locally admitted maximum-size certificate flight.
    pub fn rfc6083_sctp(mut self) -> Self {
        self.rfc6083_sctp = true;
        self.replay_detection = false;
        self.flight_retries = 0;
        self.mtu = RFC6083_DTLS_MTU;
        self
    }
    ...
    /// When set, test-visible hello randoms, cookie secrets, retry jitter, and
    /// DTLS 1.3 key-update threshold jitter are deterministic. Record AEAD
    /// nonces remain sequence-derived and are not controlled by this seed.
    ///
    /// This is useful for testing and reproducibility.
    ///
    /// Note: Cryptographic operations (key exchange, signatures) always use
    /// secure system randomness regardless of this setting. RFC 6083
    ...
    ///
    /// DTLS 1.2 has no KeyUpdate, so this is a hard per-write-key limit over
    /// every protected record, including Finished, alerts, and application
    /// data. Once exhausted, further protected sends fail before encryption or
    /// sequence-number consumption. DTLS 1.3 uses the limit to schedule
    /// KeyUpdate.
    ///
    /// Defaults to 2^23 (8,388,608); values above the 2^48 DTLS record
    /// sequence space are rejected.
    ...
    /// 5. Return [`ConfigError::NoCryptoProvider`] if no provider is available.
    pub fn build(self) -> Result<Config, Error> {
        ...
        let crypto_provider =
            crypto_provider.ok_or(Error::ConfigError(ConfigError::NoCryptoProvider))?;

        // Always validate the crypto provider
        crypto_provider.validate()?;

        if self.rfc6083_sctp && self.rng_seed.is_some() {
            return Err(Error::ConfigError(
                ConfigError::Rfc6083DeterministicRngUnsupported,
            ));
        }

        if self.handshake_timeout.is_zero() {
            return Err(Error::ConfigError(ConfigError::HandshakeTimeoutZero));
        }

        if self.require_server_certificate_request && self.psk.is_some() {
            return Err(Error::ConfigError(
                ConfigError::ServerCertificateRequestRequiredWithPsk,
            ));
        }
        ...
        if self.aead_encryption_limit > MAX_AEAD_ENCRYPTIONS {
            return Err(Error::ConfigError(
                ConfigError::AeadEncryptionLimitTooLarge {
                    limit: self.aead_encryption_limit,
                    maximum: MAX_AEAD_ENCRYPTIONS,
                },
            ));
        }
        ...
        if self.rfc6083_sctp && dtls13_count > 0 {
            return Err(Error::ConfigError(ConfigError::Rfc6083Dtls13Unsupported));
        }
        ...
        Ok(self.into_config(crypto_provider))
    }

    fn into_config(self, crypto_provider: CryptoProvider) -> Config {
        let (mtu, replay_detection, flight_retries, max_queue_rx, max_queue_tx) =
            if self.rfc6083_sctp {
                (
                    RFC6083_DTLS_MTU,
                    false,
                    0,
                    self.max_queue_rx.max(RFC6083_MIN_QUEUE_DATAGRAMS),
                    self.max_queue_tx.max(RFC6083_MIN_QUEUE_DATAGRAMS),
                )
            } else {
                (
                    self.mtu,
        ...
        Config {
            ...
            require_server_certificate_request: self.require_server_certificate_request,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Psk::Server { hint, .. } => f
                ...
                .field("hint_present", &hint.is_some())
                .field("resolver", &"...")
                .finish(),
        }
    }
...
impl fmt::Debug for Config {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Config")
            ...
            .field(
                "require_server_certificate_request",
                &self.require_server_certificate_request,
            )
            ...
            .field("rng_seed", &self.rng_seed.map(|_| "[REDACTED]"))
...
mod tests {
    ...
    #[test]
    fn server_certificate_request_policy_is_explicit_and_default_off() {
        let default = Config::builder().build().expect("default config");
        assert!(!default.require_server_certificate_request());

        let mutual_auth = Config::builder()
            .require_server_certificate_request(true)
            .build()
            .expect("mutual-auth config");
        assert!(mutual_auth.require_server_certificate_request());
    }

    #[test]
    fn server_certificate_request_policy_rejects_psk_authentication() {
        let error = Config::builder()
            .require_server_certificate_request(true)
            .with_psk_client(b"client".to_vec(), Arc::new(FixedPsk))
            .build()
            .expect_err("PSK cannot carry client certificates");

        assert!(matches!(
            error,
            Error::ConfigError(ConfigError::ServerCertificateRequestRequiredWithPsk)
        ));
    }
    ...
    fn rejects_zero_handshake_timeout() {
        assert!(matches!(
            Config::builder().handshake_timeout(Duration::ZERO).build(),
            Err(Error::ConfigError(ConfigError::HandshakeTimeoutZero))
        ));
    }
    ...
    #[test]
    fn rejects_aead_limit_beyond_dtls_sequence_space() {
        assert!(matches!(
            Config::builder()
                .aead_encryption_limit(MAX_AEAD_ENCRYPTIONS + 1)
                .build(),
            Err(Error::ConfigError(ConfigError::AeadEncryptionLimitTooLarge {
                limit,
                maximum
            })) if limit == MAX_AEAD_ENCRYPTIONS + 1 && maximum == MAX_AEAD_ENCRYPTIONS
        ));
    }

    #[test]
    fn accepts_aead_limit_equal_to_dtls_sequence_space() {
        Config::builder()
            .aead_encryption_limit(MAX_AEAD_ENCRYPTIONS)
            .build()
            .expect("the exact DTLS sequence-space limit is executable");
    }
    ...
    fn kx_groups_match_provider_when_unfiltered() {
        let config = Config::builder().build().expect("valid default config");
    ...
    #[test]
    fn rfc6083_profile_normalizes_replay_retries_and_path_mtu() {
        let config = Config::builder()
            .dtls13_cipher_suites(&[])
            .rfc6083_sctp()
            // Later generic setters must not weaken the profile.
            .replay_detection(true)
            .flight_retries(7)
            .mtu(20_000)
            .max_queue_rx(0)
        ...
        assert!(config.rfc6083_sctp());
        assert!(!config.replay_detection());
        assert_eq!(config.flight_retries(), 0);
        assert_eq!(config.mtu(), RFC6083_DTLS_MTU);
        assert_eq!(config.max_queue_rx(), RFC6083_MIN_QUEUE_DATAGRAMS);
        assert_eq!(config.max_queue_tx(), RFC6083_MIN_QUEUE_DATAGRAMS);
    }

    #[test]
    fn rfc6083_queue_normalization_is_independent_of_setter_order() {
        let before = Config::builder()
            ...
            .dtls13_cipher_suites(&[])
            .build()
            .expect("RFC 6083 config with queue setters before profile");
        let after = Config::builder()
            ...
            .dtls13_cipher_suites(&[])
            .build()
            .expect("RFC 6083 config with queue setters after profile");
    ...
    #[test]
    fn rfc6083_profile_rejects_any_surviving_dtls13_suite() {
        assert!(matches!(
            Config::builder().rfc6083_sctp().build(),
            Err(Error::ConfigError(ConfigError::Rfc6083Dtls13Unsupported))
        ));
    }
