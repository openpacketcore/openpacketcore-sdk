...
impl std::fmt::Debug for LocalEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::PeerCert => f.write_str("PeerCert"),
            Self::Connected => f.write_str("Connected"),
            Self::KeyingMaterial(_, profile) => f
                .debug_tuple("KeyingMaterial")
                .field(&"[REDACTED]")
                .field(profile)
                .finish(),
        }
...
impl Drop for LocalEvent {
    fn drop(&mut self) {
        if let Self::KeyingMaterial(material, _) = self {
            material.as_mut_slice().zeroize();
        }
    }
}
...
impl Client {
    ...
    pub(crate) fn new_from_hybrid(
        ...
        mut certificate: DtlsCertificate,
        now: Instant,
    ) -> Result<Client, Error> {
...
impl State {
    ...
    fn send_client_hello(self, client: &mut Client) -> Result<Self, InternalError> {
        ...
        let random = client.random.ok_or(Error::InvalidState(
    ...
    fn await_server_hello(self, client: &mut Client) -> Result<Self, InternalError> {
        ...
        for extension in extensions {
            if extension.extension_type == ExtensionType::UseSrtp
                && client.engine.config().rfc6083_sctp()
            {
                return Err(Error::SecurityError(
                    crate::SecurityError::UnsolicitedServerExtension {
                        extension_type: ExtensionType::UseSrtp.as_u16(),
                    },
                )
                .into());
            }
    ...
    fn await_server_key_exchange_ecdhe(self, client: &mut Client) -> Result<Self, InternalError> {
        ...
        let client_random = client.random.ok_or(Error::InvalidState(
            crate::InvalidStateError::NoClientRandom,
        ))?;
        let server_random = client.server_random.ok_or(Error::InvalidState(
    ...
    fn await_certificate_request(self, client: &mut Client) -> Result<Self, InternalError> {
        ...
        if has_done {
            if client.engine.config().require_server_certificate_request() {
                return Err(Error::SecurityError(
                    crate::SecurityError::ServerDidNotRequestClientCertificate,
                )
                .into());
            }
            return Ok(Self::AwaitServerHelloDone);
    ...
    fn await_server_hello_done(self, client: &mut Client) -> Result<Self, InternalError> {
        ...
        if client.engine.config().require_server_certificate_request() && !client.certificate_verify
        {
            return Err(Error::SecurityError(
                crate::SecurityError::ServerDidNotRequestClientCertificate,
            )
    ...
    fn send_change_cipher_spec(self, client: &mut Client) -> Result<Self, InternalError> {
        ...
        if client.engine.config().rfc6083_sctp() {
            let preceding_datagrams = client.engine.rfc6083_queued_datagrams();
            let material = client.engine.extract_rfc6083_keying_material()?;
            client
                .engine
        ...
        if client.engine.config().rfc6083_sctp() {
            client.engine.create_record_in_new_datagram(
    ...
    fn derive_keys(client: &mut Client) -> Result<(), Error> {
        ...
        client
            ...
            .serialize(&mut client_random_buf_b);
    ...
    fn await_application_data(self, client: &mut Client) -> Result<Self, InternalError> {
        if client.engine.close_notify_received() {
            ...
            if client.engine.config().rfc6083_sctp() {
                ...
                client.engine.arm_rfc6083_prepare_close_notify(0);
            } else {
                client
                    .engine
                    .create_record(ContentType::Alert, 1, false, |body| {
    ...
    match key_exchange_algorithm {
        KeyExchangeAlgorithm::EECDH => {
            ...
            let Some((curve_type, named_group)) = group_info else {
                return Err(Error::CryptoError(
                    crate::CryptoError::KeyExchangeNotInitialized,
                ));
            };
...
impl LocalEvent {
    pub fn to_output<'a>(&self, buf: &'a mut [u8], peer_certs: &[Buf]) -> Output<'a> {
        match self {
            LocalEvent::PeerCert if peer_certs.len() == 1 => {
                let cert = &peer_certs[0];
                if buf.len() < cert.len() {
                    return Output::BufferTooSmall { needed: cert.len() };
                }
                buf[..cert.len()].copy_from_slice(cert);
                Output::PeerCert(&buf[..cert.len()])
            }
            LocalEvent::PeerCert => {
                Output::PeerCertChain(peer_certs.iter().map(|cert| cert.to_vec()).collect())
            }
...
mod tests {
    ...
    fn psk_client() -> Client {
        ...
        let engine = Engine::new(Arc::new(config), AuthMode::Psk);
        Client::new_with_engine(engine, Instant::now())
    }

    fn rfc6083_psk_client() -> Client {
        let config = Config::builder()
            .rfc6083_sctp()
            .dtls13_cipher_suites(&[])
            .with_psk_client(b"client".to_vec(), Arc::new(FixedPsk))
            .build()
            .expect("valid RFC 6083 PSK config");
        let engine = Engine::new(Arc::new(config), AuthMode::Psk);
        Client::new_with_engine(engine, Instant::now())
    }

    fn client_requiring_certificate_request() -> Client {
        let config = Config::builder()
            .dtls13_cipher_suites(&[])
        ...
        let auth = AuthMode::Certificate {
            certificates: vec![vec![0x01]],
            private_key: crate::dtls12::context::unavailable_signing_key(),
        };
        Client::new_with_engine(Engine::new(Arc::new(config), auth), Instant::now())
    }
    ...
    #[test]
    fn rfc6083_client_rejects_unsolicited_server_use_srtp() {
        let mut client = rfc6083_psk_client();
        let mut extension_data = Buf::new();
        let server_hello = ServerHello::new(
            ProtocolVersion::DTLS1_2,
            Random { bytes: [0x33; 32] },
            SessionId::empty(),
        ...
        .with_extensions(
            &mut extension_data,
            Some(SrtpProfileId::SRTP_AEAD_AES_128_GCM),
        );
        let mut body = Buf::new();
        server_hello.serialize(&extension_data, &mut body);
        client
            .engine
            .parse_packet(&epoch0_handshake_packet(MessageType::ServerHello, 0, &body))
            .expect("queue syntactically valid ServerHello");

        let error = State::AwaitServerHello
            .await_server_hello(&mut client)
            .expect_err("RFC 6083 must reject RFC 5764 use_srtp");

        assert!(matches!(
            error,
            InternalError::Fatal(Error::SecurityError(
                crate::SecurityError::UnsolicitedServerExtension { extension_type: 14 }
            ))
        ));
        assert_eq!(client.negotiated_srtp_profile, None);
    }

    #[test]
    fn server_hello_done_without_required_certificate_request_fails_closed() {
        let mut client = client_requiring_certificate_request();
        client
            .engine
            .parse_packet(&epoch0_handshake_packet(
            ...
            .expect("queue ServerHelloDone without CertificateRequest");

        let error = State::AwaitCertificateRequest
            .await_certificate_request(&mut client)
            .expect_err("mutual-auth client requires CertificateRequest");
    ...
    #[test]
    fn single_maximum_certificate_is_retryable_after_small_output_buffer() {
        let mut client = psk_client();
        client
            .server_certificates
            .push(vec![0xA5; super::super::message::MAX_CERTIFICATE_BYTES].to_buf());
        client.local_events.push_back(LocalEvent::PeerCert);

        let mut bounded_record_buffer = [0_u8; 1];
        assert!(matches!(
            client.poll_output(&mut bounded_record_buffer),
            Output::BufferTooSmall {
                needed: super::super::message::MAX_CERTIFICATE_BYTES
            }
        ));
        assert_eq!(client.local_events.len(), 1);

        let mut certificate_buffer = vec![0_u8; super::super::message::MAX_CERTIFICATE_BYTES];
        assert!(matches!(
            client.poll_output(&mut certificate_buffer),
            Output::PeerCert(cert)
                if cert.len() == super::super::message::MAX_CERTIFICATE_BYTES
                    && cert.iter().all(|byte| *byte == 0xA5)
        ));
        assert!(client.local_events.is_empty());
    }
}
