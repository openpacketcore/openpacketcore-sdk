...
use crate::dtls12::context::{AuthMode, unavailable_signing_key};
...
impl Server {
    pub(crate) fn cipher_suite(&self) -> Option<Dtls12CipherSuite> {
    ...
    pub fn new(
        config: Arc<Config>,
        mut certificate: crate::DtlsCertificate,
        now: Instant,
    ) -> Server {
        let private_key = match config
            .crypto_provider()
            .key_provider
            .load_private_key(&certificate.private_key)
        {
            Ok(private_key) if !certificate.certificate.is_empty() => private_key,
            Ok(_) | Err(_) => unavailable_signing_key(),
        };
        let mut certificates = Vec::with_capacity(1 + certificate.intermediates.len());
        certificates.push(std::mem::take(&mut certificate.certificate));
        certificates.extend(std::mem::take(&mut certificate.intermediates));
    ...
    pub(crate) fn new_with_engine(mut engine: Engine, now: Instant) -> Server {
        ...
        let cookie_secret = Zeroizing::new(engine.rng.random());
    ...
    pub fn poll_output<'a>(&mut self, buf: &'a mut [u8]) -> Output<'a> {
        if let Some(event) = self.local_events.front() {
            let output = event.to_output(buf, &self.client_certificates);
    ...
    pub fn close(&mut self) -> Result<(), Error> {
        ...
        let rfc6083 = self.engine.config().rfc6083_sctp();
        let preceding_datagrams = self.engine.rfc6083_queued_datagrams();
        if rfc6083 {
            self.engine
                .create_record_in_new_datagram(ContentType::Alert, 1, false, |body| {
                    body.push(1); // level: warning
                    body.push(0); // description: close_notify
                })?;
            self.engine
                .arm_rfc6083_prepare_close_notify(preceding_datagrams);
        } else {
            self.engine
...
impl State {
    ...
    fn await_client_hello(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        if !cookie_valid {
            ...
            let cookie =
                compute_cookie(hmac_provider, server.cookie_secret.as_ref(), client_random)?;
        ...
        for ext in ch.extensions {
            match ext.extension_type {
                ExtensionType::UseSrtp if !server.engine.config().rfc6083_sctp() => {
                ...
                ExtensionType::SignatureAlgorithms => {
                    let ext_data = ext.extension_data(&server.defragment_buffer);
                    let (_, sigs) =
                        SignatureAlgorithmsExtension::parse(ext_data).map_err(|_| {
                            Error::SecurityError(
                                crate::SecurityError::MalformedClientSignatureAlgorithms,
                            )
                        })?;
        ...
        if !server.engine.config().rfc6083_sctp() {
    ...
    fn send_server_hello(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        let random = server.random.ok_or(Error::InvalidState(
            crate::InvalidStateError::NoServerRandom,
        ))?;
        let negotiated_srtp_profile = if server.engine.config().rfc6083_sctp() {
            None
        } else {
    ...
    fn send_server_key_exchange(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        let server_random = server.random.ok_or(Error::InvalidState(
            crate::InvalidStateError::NoServerRandom,
        ))?;
        ...
        let selected_signature = select_ske_signature_algorithm(
            ...
            server
                ...
                .ok_or(Error::CryptoError(
                    crate::CryptoError::NoPrivateKeyConfigured,
                ))?,
            server
                .engine
                .crypto_context()
        ...
        .ok_or(Error::SecurityError(
            crate::SecurityError::NoMutuallyAcceptableSignatureAlgorithm,
        ))?;
    ...
    fn await_client_key_exchange(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        if server.engine.config().rfc6083_sctp() {
            let preceding_datagrams = server.engine.rfc6083_queued_datagrams();
            let material = server.engine.extract_rfc6083_keying_material()?;
            server
                .engine
                .arm_rfc6083_output_barrier(preceding_datagrams, material)?;
        }
    ...
    fn await_certificate_verify(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        let (signature_range, signature_algorithm) = {
            let Some(handshake) = maybe.as_ref() else {
                return Ok(self);
            };
    ...
    fn await_finished(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        let verify_data_range = if let Body::Finished(finished) = &handshake.body {
        ...
        // the MAC above fail when the identity was unknown. Require positive
        // resolver confirmation as well so a missing state transition or a
        // future dummy-key refactor still fails closed.
        if server.engine.cipher_suite().is_some_and(|cs| cs.is_psk())
            && server.psk_valid != Some(true)
        {
            return Err((Error::SecurityError(
                crate::SecurityError::ClientFinishedVerificationFailed,
            ))
    ...
    fn send_change_cipher_spec(self, server: &mut Server) -> Result<Self, InternalError> {
        ...
        if server.engine.config().rfc6083_sctp() {
            ...
            server
                .engine
                .arm_rfc6083_prepare_epoch_barrier(preceding_datagrams.saturating_add(1))?;
        }

        // Send ChangeCipherSpec
        if server.engine.config().rfc6083_sctp() {
            server.engine.create_record_in_new_datagram(
    ...
    fn await_application_data(self, server: &mut Server) -> Result<Self, InternalError> {
        if server.engine.close_notify_received() {
            ...
            if server.engine.config().rfc6083_sctp() {
                ...
                server.engine.arm_rfc6083_prepare_close_notify(0);
            } else {
                server
                    .engine
                    .create_record(ContentType::Alert, 1, false, |body| {
...
fn select_ske_signature_algorithm(
    ...
    // RFC 5246 §7.4.1.4.1 defines SHA-1 as the pre-extension default.
    // Apply it only if the configured signing implementation can execute it.
    supported_hashes
        .contains(&HashAlgorithm::SHA1)
        .then(|| SignatureAndHashAlgorithm::new(HashAlgorithm::SHA1, our_sig))
}
...
mod tests {
    ...
    fn epoch0_handshake_packet(msg_type: MessageType, message_seq: u16, body: &[u8]) -> Vec<u8> {
        ...
        packet.extend_from_slice(&ProtocolVersion::DTLS1_2.as_u16().to_be_bytes());
        packet.extend_from_slice(&0_u16.to_be_bytes());
        packet.extend_from_slice(&0_u64.to_be_bytes()[2..]);
        ...
        packet.extend_from_slice(&0_u32.to_be_bytes()[1..]);
        packet.extend_from_slice(&(body.len() as u32).to_be_bytes()[1..]);
        packet.extend_from_slice(body);
        packet
    }

    fn client_hello_body(cipher_suite: Dtls12CipherSuite, extensions: &[u8]) -> Vec<u8> {
        let mut body = Vec::new();
        body.extend_from_slice(&ProtocolVersion::DTLS1_2.as_u16().to_be_bytes());
        body.extend_from_slice(&[0x44; 32]);
        body.push(0); // empty session ID
        body.push(0); // empty cookie
        body.extend_from_slice(&2_u16.to_be_bytes());
        body.extend_from_slice(&cipher_suite.as_u16().to_be_bytes());
        body.push(1);
    ...
    fn extension(extension_type: ExtensionType, data: &[u8], output: &mut Vec<u8>) {
        output.extend_from_slice(&extension_type.as_u16().to_be_bytes());
        output.extend_from_slice(&(data.len() as u16).to_be_bytes());
        output.extend_from_slice(data);
    }

    fn psk_server(rfc6083: bool) -> Server {
        let builder = Config::builder()
            .dtls13_cipher_suites(&[])
            .use_server_cookie(false)
            .with_psk_server(None, Arc::new(FixedPsk));
        let config = if rfc6083 {
            builder.rfc6083_sctp()
        } else {
            builder
        }
        .build()
        .expect("valid PSK server config");
        Server::new_psk(Arc::new(config), Instant::now())
    }
    ...
    #[test]
    fn ske_signature_selection_rejects_present_extension_without_overlap() {
    ...
    #[test]
    fn ske_signature_selection_uses_only_supported_rfc_default_when_absent() {
        assert_eq!(
            select_ske_signature_algorithm(
                None,
                SignatureAlgorithm::ECDSA,
    ...
    #[test]
    fn malformed_client_signature_algorithms_is_terminal_and_typed() {
        let mut extensions = Vec::new();
        extension(ExtensionType::ExtendedMasterSecret, &[], &mut extensions);
        extension(
            ExtensionType::SignatureAlgorithms,
            &[0, 1, HashAlgorithm::SHA256.as_u8()],
            &mut extensions,
        );
        let body = client_hello_body(Dtls12CipherSuite::PSK_AES128_CCM_8, &extensions);
        let mut server = psk_server(false);
        server
            .engine
            .parse_packet(&epoch0_handshake_packet(MessageType::ClientHello, 0, &body))
            .expect("queue malformed-signature ClientHello");

        let error = State::AwaitClientHello
            .await_client_hello(&mut server)
            .expect_err("malformed signature_algorithms must be terminal");

        assert!(matches!(
            error,
            InternalError::Fatal(Error::SecurityError(
                crate::SecurityError::MalformedClientSignatureAlgorithms
            ))
        ));
    }
    ...
    #[test]
    fn required_empty_client_certificate_is_terminal() {
        let mut server = psk_server(false);
        server
            .engine
            .set_cipher_suite(Dtls12CipherSuite::ECDHE_ECDSA_AES128_GCM_SHA256)
            .expect("select certificate suite");
        server
            .engine
        ...
        let error = State::AwaitCertificate
            .await_certificate(&mut server)
            .expect_err("required client certificate cannot be empty");

        assert!(matches!(
            error,
            InternalError::Fatal(Error::CertificateError(
                crate::CertificateError::NoClientCertificateForVerification
            ))
        ));
        assert!(server.client_certificates.is_empty());
        assert!(server.local_events.is_empty());
    }

    #[test]
    fn rfc6083_server_ignores_offered_use_srtp_and_never_echoes_it() {
        let mut extensions = Vec::new();
        extension(ExtensionType::UseSrtp, &[0, 2, 0, 7, 0], &mut extensions);
        extension(ExtensionType::ExtendedMasterSecret, &[], &mut extensions);
        let body = client_hello_body(Dtls12CipherSuite::PSK_AES128_CCM_8, &extensions);
        let mut server = psk_server(true);
        server
            .engine
            .parse_packet(&epoch0_handshake_packet(MessageType::ClientHello, 0, &body))
            .expect("queue RFC 6083 ClientHello");

        let state = State::AwaitClientHello
            .await_client_hello(&mut server)
            .expect("RFC 6083 ignores RFC 5764 offer");
        assert_eq!(state, State::SendServerHello);
        assert_eq!(server.negotiated_srtp_profile, None);

        server.random = Some(Random { bytes: [0x55; 32] });
        let state = state
            .send_server_hello(&mut server)
            .expect("serialize RFC 6083 ServerHello");
        assert_eq!(state, State::SendServerKeyExchange);

        let mut packet_buffer = vec![0_u8; crate::config::RFC6083_DTLS_MTU];
        let packet = match server
            .engine
            .poll_output(&mut packet_buffer, Instant::now())
        {
            Output::Packet(packet) => packet,
            output => panic!("expected ServerHello packet, got {output:?}"),
        };
        let (_, server_hello) = ServerHello::parse(&packet[DTLSRecord::HEADER_LEN + 12..], 0)
            .expect("parse emitted ServerHello");
        assert!(
            server_hello
                .extensions
                .expect("ServerHello extensions")
                .iter()
                .all(|item| item.extension_type != ExtensionType::UseSrtp)
        );
    }

    #[test]
    #[cfg(feature = "rcgen")]
    fn present_signature_algorithms_without_overlap_fails_closed() {
        ...
        extension(
            ...
            &[
                0,
                2,
                HashAlgorithm::SHA256.as_u8(),
                SignatureAlgorithm::RSA.as_u8(),
            ],
            &mut extensions,
        );
        ...
        let config = Arc::new(
            Config::builder()
                ...
                .use_server_cookie(false)
                .build()
                .expect("valid certificate server config"),
        );
        let certificate = generate_self_signed_certificate().expect("generate certificate");
        let now = Instant::now();
        let mut server = Server::new(config, certificate, now);
        server.random = Some(Random { bytes: [0x66; 32] });

        let error = server
            .handle_packet(&epoch0_handshake_packet(MessageType::ClientHello, 0, &body))
            .expect_err("present signature list without overlap must fail");

        assert!(matches!(
            error,
            Error::SecurityError(crate::SecurityError::NoMutuallyAcceptableSignatureAlgorithm)
        ));
    }
    ...
    #[test]
    fn single_maximum_client_certificate_is_retryable_after_small_output_buffer() {
        let config = Arc::new(Config::builder().build().expect("valid default config"));
        let mut server = Server::new_psk(config, Instant::now());
        server
            .client_certificates
            .push(vec![0x5A; crate::dtls12::message::MAX_CERTIFICATE_BYTES].to_buf());
        server.local_events.push_back(LocalEvent::PeerCert);

        let mut bounded_record_buffer = [0_u8; 1];
        assert!(matches!(
            server.poll_output(&mut bounded_record_buffer),
            Output::BufferTooSmall {
                needed: crate::dtls12::message::MAX_CERTIFICATE_BYTES
            }
        ));
        assert_eq!(server.local_events.len(), 1);

        let mut certificate_buffer = vec![0_u8; crate::dtls12::message::MAX_CERTIFICATE_BYTES];
        assert!(matches!(
            server.poll_output(&mut certificate_buffer),
            Output::PeerCert(cert)
                if cert.len() == crate::dtls12::message::MAX_CERTIFICATE_BYTES
                    && cert.iter().all(|byte| *byte == 0x5A)
        ));
        assert!(server.local_events.is_empty());
    }
}
