...
impl State {
    ...
    fn send_certificate(self, client: &mut Client) -> Result<Self, InternalError> {
        ...
        if has_cert {
            ...
            context_copy.try_extend_from_slice(context).map_err(|_| {
...
pub(crate) fn handshake_create_certificate(
    ...
    let mut encoded_chain_len = 0_usize;
    for certificate_der in engine.certificate_chain_der() {
        if certificate_der.is_empty() || certificate_der.len() > 0x00ff_ffff {
            return Err(Error::CryptoError(
                crate::CryptoError::CertificateChainTooLarge,
            ));
        }
        encoded_chain_len = encoded_chain_len
            .checked_add(3 + certificate_der.len() + 2)
            .ok_or(Error::CryptoError(
                crate::CryptoError::CertificateChainTooLarge,
            ))?;
        if encoded_chain_len > 0x00ff_ffff {
            return Err(Error::CryptoError(
                crate::CryptoError::CertificateChainTooLarge,
            ));
        }
        let cert_start = src.len();
        src.extend_from_slice(certificate_der);
        ...
        certificate_list
            ...
            .map_err(|_| Error::CryptoError(crate::CryptoError::CertificateChainTooLarge))?;
    }
...
mod tests {
    ...
    impl ActiveKeyExchange for TestKeyExchange {
        ...
        fn complete(self: Box<Self>, _peer_pub: &[u8], _out: &mut Buf) -> Result<(), CryptoError> {
            Err(CryptoError::InvalidPublicKey(self.group))
        }
    ...
    fn client() -> Client {
        ...
        let engine = Engine::new(Arc::new(config), cert);
    ...
    #[test]
    fn required_certificate_request_cannot_be_skipped_by_server_certificate() {
        ...
        client
            ...
            .expect("queue Certificate without CertificateRequest");

        let error = State::AwaitCertificateRequest
            .await_certificate_request(&mut client)
            .expect_err("required CertificateRequest must not be skipped");

        assert!(matches!(
            error,
            InternalError::Fatal(Error::SecurityError(
                SecurityError::ServerDidNotRequestClientCertificate
            ))
        ));
    }

    #[test]
    fn peer_certificate_event_survives_small_buffer_retry() {
        let mut client = client();
        client.server_certificates.push(vec![0xA5; 2048].to_buf());
        client.local_events.push_back(LocalEvent::PeerCert);

        let mut small = [0_u8; 1];
        assert!(matches!(
            client.poll_output(&mut small),
            Output::BufferTooSmall { needed: 2048 }
        ));
        assert_eq!(client.local_events.len(), 1);

        let mut large = vec![0_u8; 2048];
        assert!(matches!(
            client.poll_output(&mut large),
            Output::PeerCert(cert) if cert == vec![0xA5; 2048]
        ));
