...
impl Server {
    ...
    pub fn new_with_engine(mut engine: Engine, now: Instant, auto_mode: bool) -> Server {
        let cookie_secret = Zeroizing::new(engine.random_arr());
...
fn handshake_create_server_hello(
    ...
    let cipher_suite = engine.cipher_suite().ok_or(Error::InvalidState(
...
mod tests {
    ...
    fn peer_certificate_event_survives_small_buffer_retry() {
        let certificate =
            crate::certificate::generate_self_signed_certificate().expect("generate certificate");
        let config = Arc::new(Config::builder().build().expect("valid default config"));
        let mut server = Server::new(config, certificate, Instant::now());
        server.client_certificates.push(vec![0x5A; 2048].to_buf());
        server.local_events.push_back(LocalEvent::PeerCert);

        let mut small = [0_u8; 1];
        assert!(matches!(
            server.poll_output(&mut small),
        ...
        assert!(matches!(
            server.poll_output(&mut large),
            Output::PeerCert(cert) if cert == vec![0x5A; 2048]
        ));
