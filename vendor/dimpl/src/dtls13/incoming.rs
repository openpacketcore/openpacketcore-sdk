    ...
    pub fn parse(
        ...
        while !packet.is_empty() {
            let record_end = if Dtls13Record::is_ciphertext_header(packet[0]) {
                ...
                if l_flag {
                    ...
                    let length_bytes = [packet[len_offset], packet[len_offset + 1]];
                ...
                let length_offset = Dtls13Record::PLAINTEXT_LENGTH_OFFSET.start;
                let length_bytes = [packet[length_offset], packet[length_offset + 1]];
                let length = u16::from_be_bytes(length_bytes) as usize;
        ...
        for record in parsed_records {
            if let Some(record) = decrypt.classify_record(record)? {
                records
                    .try_push(record)
                    .map_err(|_| InternalError::too_many_records())?;
            }
        }
    ...
    pub fn parse(
        ...
        if is_ciphertext && decrypt.is_peer_encryption_enabled() {
            ...
            if buffer.len() >= header_len + 16 {
                ...
                ciphertext_sample.copy_from_slice(&buffer[header_len..header_len + 16]);
