...
impl Engine {
    pub fn new(config: Arc<Config>, certificate: DtlsCertificate) -> Self {
        ...
        let signing_key = config
            ...
            .unwrap_or_else(|_| crate::crypto::unavailable_signing_key());
    ...
    pub fn certificate_chain_der(&self) -> impl Iterator<Item = &[u8]> {
        std::iter::once(self.certificate.certificate.as_slice())
            .chain(self.certificate.intermediates.iter().map(Vec::as_slice))
    }
    ...
    fn insert_incoming_handshake(&mut self, incoming: Incoming) -> Result<(), Error> {
        ...
        let handshake = first_record.first_handshake().ok_or(Error::InvalidState(
        ...
        F: FnOnce(&mut Buf),
    {
        ...
        if can_append {
            let last = self.queue_tx.back_mut().ok_or(Error::InvalidState(
        ...
        F: FnOnce(&mut Buf),
    {
        ...
        let sn_mask = if fragment.len() >= 16 {
            let mut ciphertext_sample = [0u8; 16];
            ciphertext_sample.copy_from_slice(&fragment[..16]);
    ...
    fn process_ack(&mut self, ack_data: &[u8]) -> Result<(), Error> {
        ...
        for i in 0..num_entries {
            ...
            epoch_bytes.copy_from_slice(&entries_data[offset..offset + 8]);
            let ack_epoch = u64::from_be_bytes(epoch_bytes);
            let mut seq_bytes = [0u8; 8];
            seq_bytes.copy_from_slice(&entries_data[offset + 8..offset + 16]);
            let ack_seq = u64::from_be_bytes(seq_bytes);
    ...
    fn hash_algorithm(&self) -> Result<HashAlgorithm, Error> {
        self.cipher_suite
            .map(|suite| suite.hash_algorithm())
            .ok_or(Error::InvalidState(
                crate::InvalidStateError::NoCipherSuiteSelected,
            ))
    }

    fn suite_provider(&self) -> Result<&'static dyn SupportedDtls13CipherSuite, Error> {
        let suite = self.cipher_suite.ok_or(Error::InvalidState(
            crate::InvalidStateError::NoCipherSuiteSelected,
        ))?;
        self.config
            .crypto_provider()
    ...
    pub fn transcript_hash(&self, out: &mut Buf) -> Result<(), Error> {
        let hash = self.hash_algorithm()?;
        let mut ctx = self
            .config
            .crypto_provider()
    ...
    fn transcript_hash_of(&self, data: &[u8]) -> Result<Buf, Error> {
        let hash = self.hash_algorithm()?;
        let mut ctx = self
            .config
            .crypto_provider()
    ...
    pub fn replace_transcript_with_message_hash(&mut self, split_at: usize) -> Result<(), Error> {
        let hash = self.hash_algorithm()?;
...
impl RecordHandler for Engine {
    ...
    fn min_protected_fragment_len(&self) -> usize {
        self.suite_provider()
            .map(SupportedDtls13CipherSuite::min_protected_fragment_len)
            .unwrap_or(usize::MAX)
    }
    ...
    fn decrypt_sequence_number(
        ...
        let Ok(suite) = self.suite_provider() else {
            return;
        };
        let mask = suite.encrypt_sn(sn_key, ciphertext_sample);
...
mod tests {
    ...
    fn derive_handshake_secrets_returns_handshake_secret() {
        ...
        let hash = engine.hash_algorithm().unwrap();
        let hash_len = hash.output_len();
        let hmac = engine.hmac();

        let mut transcript_hash = Buf::new();
        engine.transcript_hash(&mut transcript_hash).unwrap();
