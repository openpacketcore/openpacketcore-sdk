...
pub(crate) struct ClientPending {
    ...
    last_now: Instant,
    /// Absolute deadline for the hybrid ClientHello phase.
...
impl ClientPending {
    ...
    pub fn handle_timeout(&mut self, now: Instant) -> Result<(), Error> {
        ...
        if now >= self.handshake_deadline {
            return Err(Error::Timeout(TimeoutError::Connect));
        }
    ...
    pub fn poll_output<'a>(&mut self, buf: &'a mut [u8]) -> Output<'a> {
        ...
        let next = if self.config.rfc6083_sctp() {
