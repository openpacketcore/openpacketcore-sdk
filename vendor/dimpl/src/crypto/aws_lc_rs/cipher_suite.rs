...
impl SupportedDtls13CipherSuite for Tls13ChaCha20Poly1305Sha256 {
    ...
    fn encrypt_sn(&self, sn_key: &[u8], sample: &[u8; 16]) -> [u8; 16] {
        ...
        let Ok(hp_key) = HeaderProtectionKey::new(&aws_quic::CHACHA20, sn_key) else {
            return [0; 16];
        };
        let Ok(mask) = hp_key.new_mask(sample) else {
            return [0; 16];
        };
        let mut out = [0u8; 16];
...
fn aes_ecb_encrypt(
    ...
    let Ok(unbound) = UnboundCipherKey::new(algorithm, key) else {
        return [0; 16];
    };
    let Ok(ecb_key) = EncryptingKey::ecb(unbound) else {
        return [0; 16];
    };
    let mut block = *input;
    if ecb_key.encrypt(&mut block).is_err() {
        return [0; 16];
    }
    block
}
