...
use super::{
    Aad, CryptoProvider, Dtls12CipherSuite, Nonce, SupportedDtls12CipherSuite, SupportedKxGroup,
};
...
impl CryptoProvider {
    ...
    /// Validate the declared metadata and AEAD operation for every DTLS 1.2
    /// suite the provider makes negotiable.
    fn validate_dtls12_aead(&self) -> Result<(), Error> {
        for cs in self.supported_cipher_suites() {
            let suite = cs.suite();
            let parameters = dtls12_parameters(suite).ok_or_else(|| {
                provider_error(CryptoProviderValidationError::NoDtls12AeadTestVector(suite))
            })?;
            if cs.hash_algorithm() != parameters.hash
                || cs.key_lengths() != parameters.key_lengths
                || cs.explicit_nonce_len() != parameters.explicit_nonce_len
                || cs.tag_len() != parameters.tag_len
                || cs.min_protected_fragment_len() != parameters.min_protected_fragment_len
            {
                return Err(provider_error(
                    CryptoProviderValidationError::Dtls12CipherSuiteParametersInvalid(suite),
                ));
            }

            let vector = dtls12_aead_test_vector(suite).ok_or_else(|| {
                provider_error(CryptoProviderValidationError::NoDtls12AeadTestVector(suite))
            })?;
            let nonce = Nonce(vector.nonce);
            let mut aad_bytes = ArrayVec::new();
            if aad_bytes.try_extend_from_slice(vector.aad).is_err() {
                return Err(provider_error(
                    CryptoProviderValidationError::Dtls12AeadAdditionalDataTooLong(suite),
                ));
            }
            let aad = Aad(aad_bytes);

            let mut cipher = cs.create_cipher(vector.key).map_err(|source| {
                provider_error(CryptoProviderValidationError::Dtls12AeadCreateFailed {
                    suite,
                    source,
                })
            })?;
            let mut encrypted = Buf::new();
            encrypted.extend_from_slice(vector.plaintext);
            cipher
                .encrypt(&mut encrypted, aad.clone(), nonce)
                .map_err(|source| {
                    provider_error(CryptoProviderValidationError::Dtls12AeadEncryptFailed {
                        suite,
                        source,
                    })
                })?;
            if encrypted.as_ref() != vector.ciphertext_tag {
                return Err(provider_error(
                    CryptoProviderValidationError::Dtls12AeadEncryptWrongOutput(suite),
                ));
            }
            ...
            let mut ciphertext = Vec::from(vector.ciphertext_tag);
            let mut ciphertext = TmpBuf::new(&mut ciphertext);
            cipher
                .decrypt(&mut ciphertext, aad, nonce)
                .map_err(|source| {
                    provider_error(CryptoProviderValidationError::Dtls12AeadDecryptFailed {
                        suite,
                        source,
                    })
                })?;
            if ciphertext.as_ref() != vector.plaintext {
                return Err(provider_error(
                    CryptoProviderValidationError::Dtls12AeadDecryptWrongOutput(suite),
                ));
            }
        }
        Ok(())
    ...
    fn validate_dtls13_aead(&self) -> Result<(), Error> {
        for cs in self.dtls13_cipher_suites {
            ...
            if aad_vec.try_extend_from_slice(tv.aad).is_err() {
                return Err(provider_error(
                    CryptoProviderValidationError::AeadAdditionalDataTooLong(suite),
                ));
            }
            let aad = Aad(aad_vec);
...
#[derive(Clone, Copy)]
struct Dtls12Parameters {
    hash: HashAlgorithm,
    key_lengths: (usize, usize, usize),
    explicit_nonce_len: usize,
    tag_len: usize,
    min_protected_fragment_len: usize,
}

fn dtls12_parameters(suite: Dtls12CipherSuite) -> Option<Dtls12Parameters> {
    match suite {
        Dtls12CipherSuite::ECDHE_ECDSA_AES128_GCM_SHA256 => Some(Dtls12Parameters {
            hash: HashAlgorithm::SHA256,
            key_lengths: (0, 16, 4),
            explicit_nonce_len: 8,
            tag_len: 16,
            min_protected_fragment_len: 24,
        }),
        Dtls12CipherSuite::ECDHE_ECDSA_AES256_GCM_SHA384 => Some(Dtls12Parameters {
        ...
        Dtls12CipherSuite::ECDHE_ECDSA_CHACHA20_POLY1305_SHA256 => Some(Dtls12Parameters {
            hash: HashAlgorithm::SHA256,
            key_lengths: (0, 32, 12),
            explicit_nonce_len: 0,
            tag_len: 16,
            min_protected_fragment_len: 16,
        }),
        Dtls12CipherSuite::PSK_AES128_CCM_8 => Some(Dtls12Parameters {
            hash: HashAlgorithm::SHA256,
            key_lengths: (0, 16, 4),
            explicit_nonce_len: 8,
            tag_len: 8,
...
struct Dtls12AeadTestVector {
    key: &'static [u8],
    nonce: [u8; 12],
    plaintext: &'static [u8],
    aad: &'static [u8],
    ciphertext_tag: &'static [u8],
}

fn dtls12_aead_test_vector(suite: Dtls12CipherSuite) -> Option<Dtls12AeadTestVector> {
    let dtls13_suite = match suite {
        ...
        Dtls12CipherSuite::ECDHE_ECDSA_AES256_GCM_SHA384 => {
            Some(Dtls13CipherSuite::AES_256_GCM_SHA384)
        }
        Dtls12CipherSuite::ECDHE_ECDSA_CHACHA20_POLY1305_SHA256 => {
            Some(Dtls13CipherSuite::CHACHA20_POLY1305_SHA256)
        }
        Dtls12CipherSuite::PSK_AES128_CCM_8 => None,
        _ => return None,
    };

    if let Some(dtls13_suite) = dtls13_suite {
        let vector = AEAD_TEST_VECTORS
            .iter()
            .find(|vector| vector.suite == dtls13_suite)?;
        return Some(Dtls12AeadTestVector {
            key: vector.key,
            nonce: vector.nonce,
            plaintext: vector.plaintext,
            aad: vector.aad,
            ciphertext_tag: vector.ciphertext_tag,
        });
    }

    // AES-128-CCM with an 8-byte tag. This exact key/nonce/AAD/plaintext
    // tuple and ciphertext||tag was independently reproduced with Python
    // cryptography's AESCCM(tag_length=8), backed by OpenSSL.
    Some(Dtls12AeadTestVector {
        key: &[
        ...
        ciphertext_tag: &[
            0x03, 0x94, 0x9b, 0x83, 0x66, 0xd1, 0x0b, 0x95, 0x22, 0x85, 0x55, 0xeb, 0x58, 0x49,
            0xf5, 0xa9, 0x4b, 0x86, 0xe0, 0x84, 0x01, 0x72, 0x3c, 0x1c, 0x86, 0x97, 0xbb, 0xcd,
            0xfc, 0x13, 0xee, 0x12,
        ],
    })
}
...
#[cfg(test)]
#[derive(Debug)]
struct BrokenDtls12Cipher;

#[cfg(test)]
impl super::Cipher for BrokenDtls12Cipher {
    fn encrypt(
        &mut self,
        _plaintext: &mut Buf,
        _aad: Aad,
...
#[cfg(test)]
#[derive(Debug)]
struct BrokenDtls12Suite {
    bad_metadata: bool,
}

#[cfg(test)]
impl SupportedDtls12CipherSuite for BrokenDtls12Suite {
    ...
    fn create_cipher(&self, _key: &[u8]) -> Result<Box<dyn super::Cipher>, crate::CryptoError> {
        Ok(Box::new(BrokenDtls12Cipher))
    }
}

#[cfg(test)]
static BROKEN_OPERATION_SUITE: BrokenDtls12Suite = BrokenDtls12Suite {
    bad_metadata: false,
};
#[cfg(test)]
static BROKEN_METADATA_SUITE: BrokenDtls12Suite = BrokenDtls12Suite { bad_metadata: true };
#[cfg(test)]
static BROKEN_OPERATION_SUITES: &[&dyn SupportedDtls12CipherSuite] = &[&BROKEN_OPERATION_SUITE];
#[cfg(test)]
static BROKEN_METADATA_SUITES: &[&dyn SupportedDtls12CipherSuite] = &[&BROKEN_METADATA_SUITE];

#[cfg(test)]
fn assert_broken_dtls12_provider_is_rejected(mut provider: CryptoProvider) {
    provider.cipher_suites = BROKEN_OPERATION_SUITES;
    assert!(matches!(
        crate::Config::builder()
            .with_crypto_provider(provider)
            .build(),
        Err(Error::ConfigError(ConfigError::CryptoProvider(
            CryptoProviderValidationError::Dtls12AeadEncryptWrongOutput(
                Dtls12CipherSuite::ECDHE_ECDSA_AES128_GCM_SHA256
            )
        )))
    ));
...
#[cfg(test)]
fn assert_bad_dtls12_metadata_is_rejected(mut provider: CryptoProvider) {
    provider.cipher_suites = BROKEN_METADATA_SUITES;
    assert!(matches!(
        crate::Config::builder()
            .with_crypto_provider(provider)
            .build(),
        Err(Error::ConfigError(ConfigError::CryptoProvider(
            CryptoProviderValidationError::Dtls12CipherSuiteParametersInvalid(
                Dtls12CipherSuite::ECDHE_ECDSA_AES128_GCM_SHA256
            )
        )))
    ));
...
mod tests_aws_lc_rs {
    ...
    #[test]
    fn config_rejects_non_executable_dtls12_provider() {
        assert_broken_dtls12_provider_is_rejected(aws_lc_rs::default_provider());
    }

    #[test]
    fn config_rejects_false_dtls12_suite_metadata() {
        assert_bad_dtls12_metadata_is_rejected(aws_lc_rs::default_provider());
    }
...
mod tests_rust_crypto {
    ...
    #[test]
    fn config_rejects_non_executable_dtls12_provider() {
        assert_broken_dtls12_provider_is_rejected(rust_crypto::default_provider());
    }

    #[test]
    fn config_rejects_false_dtls12_suite_metadata() {
        assert_bad_dtls12_metadata_is_rejected(rust_crypto::default_provider());
    }
