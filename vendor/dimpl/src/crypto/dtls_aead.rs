...
impl Iv {
    pub(crate) fn new(iv: &[u8]) -> Result<Self, crate::CryptoError> {
        if iv.len() > 12 {
            return Err(crate::CryptoError::InvalidNonce);
        }
    ...
    pub(crate) fn as_12_bytes(&self) -> Result<&[u8; 12], crate::CryptoError> {
        if self.len() == 12 {
            Ok(&self.bytes)
        } else {
            Err(crate::CryptoError::InvalidNonce)
        }
    }
...
impl Nonce {
    ...
    pub(crate) fn new(iv: Iv, explicit_nonce: &[u8]) -> Result<Self, crate::CryptoError> {
        if iv.len() + explicit_nonce.len() != 12 {
            return Err(crate::CryptoError::InvalidNonce);
        }
...
impl Aad {
    ...
    pub(crate) fn new_dtls13(header_bytes: &[u8]) -> Self {
        let mut aad = ArrayVec::new();
        // Unified DTLS headers are at most five bytes. Truncating to the AAD
        // type's capacity keeps this constructor total if a future caller
        // violates that internal contract.
        for byte in header_bytes.iter().copied().take(aad.capacity()) {
            aad.push(byte);
        }
        Aad(aad)
    }
