...
//! // Use the default aws-lc-rs provider (implicit and validated)
...
impl SigningKey for UnavailableSigningKey {
    fn sign(
        &mut self,
        _data: &[u8],
...
/// A fail-closed signing key used by infallible constructors when local key
/// loading fails. Any attempted signature returns `InvalidPrivateKey`.
pub(crate) fn unavailable_signing_key() -> Box<dyn SigningKey> {
...
impl CryptoProvider {
    ...
    ///
    /// Returns the supplied provider if a process-wide default was already installed.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// # fn main() {}
    /// ```
    // Preserve OnceLock::set's useful ownership contract: on failure callers
    // receive the provider they attempted to install.
