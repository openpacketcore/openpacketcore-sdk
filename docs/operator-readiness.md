...
|:---|:---|:---|
| Experimental protocol crates | `opc-proto-gtpv2c`, `opc-proto-diameter`, and `opc-proto-ikev2` provide bounded codec mechanisms, typed Rf/SWm and IKE_AUTH helper subsets, typed executable IKE-SA profiles/KDFs, product-neutral SA_INIT proposal selection, AES-GCM/AES-CBC `SK`/`SKF` protection, explicit ENCR_NULL authenticated-only Child-SA profiles with zero encryption KEYMAT, RFC 7383 fragment framing/reassembly helpers, conformance notes, hostile-input checks, and fuzz targets that downstream product tests can call before entering simulator or operator policy paths. | The crates do not enable ENCR_NULL or choose any algorithm allowlist. They also do not provide UDP peer lifecycle, realm routing, AAA/HSS/CDF behavior, IKE SA/EAP-AKA state, retransmission timers/caches or fragment queues, Child SA lifecycle policy, or carrier acceptance evidence. They are not default `opc-sdk` facade exports. |
| XFRM/IPsec backend | `opc-ipsec-xfrm` provides safe XFRM request models, a Linux backend, production fixed outer-DSCP stamping through a validated XFRM output-mark/tc companion, a deterministic mock backend, rollback-aware SA+policy composites, an exact current-upstream single-SA outer-endpoint/NAT-T relocation primitive, and an opt-in IKEv2 Child SA to XFRM request mapper. The mapper represents authenticated-only ESP with Linux's required zero-key NULL cipher plus separate auth attribute; a privileged namespace test proves valid bidirectional traffic and tamper rejection. | Products still own authenticated relocation signalling, namespace-wide writer serialization, and XFRM policy/template coordination. Outgoing relocation must install a temporary block policy, remove the old allow policy, migrate, install the replacement allow policy, and only then remove the block; this prevents cleartext fallback and AES-GCM IV reuse. Incoming relocation does not require that block. Products also own algorithm/profile policy (including whether ENCR_NULL is permitted), namespace and privilege rendering, the globally reserved seven-bit skb-mark window, complete SWu egress-interface configuration, kernel/UAPI rollout, traffic readiness, seamless-mobility evidence, and Child SA lifecycle policy; the transport-neutral IKE key derivation mechanism is SDK-owned by `opc-proto-ikev2`. |
...
netlink worker can outlive a dropped Rust future, client disconnect, or aborting
timeout. The product supervisor must poll the operation to completion. If task
or process completion is uncertain, treat the result as `StateIndeterminate`,
retain the outbound block policy and namespace-wide writer exclusion, and
reconcile the exact old and new SA tuples before releasing either fence. After
process loss, recovery performs the same reconciliation before retrying;
relocation is not blindly idempotent. Startup should call
`sa_relocation_capability`: its collision-free missing-SA probe distinguishes
supported (`ESRCH`), pre-UAPI (`EINVAL`), and feature-disabled
(`ENOPROTOOPT`) kernels without inferring support from a version string.
...
`88c8627c35d96d80cb304d3f50fadc8c1b21a5cb`, not registry 0.9.24. Both domains
...
| Evidence format | `opc-evidence` provides tested RFC 006 record, manifest, gap, SBOM/VEX, provenance, performance, bundle, and policy-evaluation library primitives. Bundle signing bytes are deterministic and domain-separated; release verification binds the authenticated signer identity, separately supplied artifact bytes, canonical record/gap/waiver gate inputs, and consistent commit identities. Repository workflows do not yet invoke the evaluator or wire a production external signer/verifier and complete artifact set. | `crates/opc-evidence/src/extract.rs`, `crates/opc-evidence/src/sbom.rs`, `crates/opc-evidence/src/vex.rs`, `crates/opc-evidence/src/provenance.rs`, `crates/opc-evidence/src/bundle.rs`, `crates/opc-evidence/src/performance.rs`, `crates/opc-evidence/src/policy.rs`, `crates/opc-evidence/tests/evidence_bundle.rs`, `crates/opc-evidence/tests/evidence_policy.rs`, `docs/implementation-status.md#known-gaps` (`GAP-006-*`) |
...
- **Narrowed / Partial**:
  ...
  - RFC 006 evidence primitives are implemented as library APIs and signed
    artifact substitution is rejected, but the full candidate artifact set,
    production external signer/verifier wiring, and workflow enforcement are
    incomplete.
- **Open / Remaining Gaps**:
