...
July 2026 addendum for AC 005-6 and 005-8: `opc-proto-tft` and
`opc-proto-eap` follow the same protocol-crate layout and add stable corpus
replay plus registered fuzz targets. Their conformance sources are
[`crates/opc-proto-tft/CONFORMANCE.md`](../crates/opc-proto-tft/CONFORMANCE.md)
and
[`crates/opc-proto-eap/CONFORMANCE.md`](../crates/opc-proto-eap/CONFORMANCE.md).
...
| GAP-PROTO-003 | 005 | NGAP codec (ASN.1 APER) | partial — [`ADR 0013`](adr/0013-ngap-asn1-strategy.md) | `opc-proto-ngap` | Root container construction and canonical encoding match independent Release 18 evidence for 23 admitted outcomes (15 in the published corpus). Bounded fragmentation avoids generated framing defects; raw preservation and existing decode policies remain available. Typed N3IWF fields, Initial UE/Downlink NAS/Uplink NAS, UE Release Command/Complete, and all three NG Setup outcomes have independent required-field evidence. Setup covers bounded root PLMN/slice/GUAMI lists and preserves receiver-ignore DRX/IAB behavior. Standalone GUAMI/Allowed NSSAI and security-mask construction have independent field evidence. Nested setup-request transfers cover distinct UL/DL tunnel fields and unique non-GBR 5QI 9 flows with conditional session AMBR; single-downlink response/partial-QFI results and root-Cause unsuccessful transfers also have independent evidence. Seven outer context/PDU Setup list roots admit bounded optional NAS, slices and contained transfers with unique session IDs and disjoint partial results. All five Initial Context/PDU Session Resource Setup outcomes compose those fields with required/conditional presence, explicit receiver-ignore behavior and nested caller policies, backed by 122 independent complete-message cases. Keys remain borrowed; request correlation and resource effects remain caller-owned. PDU Resource Release Command/Response, both contained transfers and unique session lists have independent evidence for all root Causes and every list length; teardown remains caller-owned. NAS Non-Delivery Indication and UE Context Release Request add 291 independent complete-message cases and a bounded unique session-ID list; shared root Cause admission rejects nonzero padding. Reset/Reset Acknowledge/Error Indication add 1,093 independent field cases and 189 complete messages, explicit signalling context, bounded fragmented connection lists with receiver-ignored empty items, and root Criticality Diagnostics; correlation, triggers and resource effects remain caller-owned. PDU Resource Notify adds 971 independent fields and 43 complete messages for root flow/session reports, disjoint identifiers and optional location; callers verify established GBR classification and ownership. Shared contained-field preflight rejects nonminimal short-value lengths. Standalone Modify root fields add 687 independent cases for optional-parameter requests, QFI/Cause reports and directional tunnel pairs, with complete preflight before allocation. Remaining: applicable optional fields, other mandatory/conditional presence rules, pending codecs beyond the explicit N3IWF applicability/receive/error/trigger matrix, and live interoperability evidence (#787). Modify Request Transfer composes four optional root fields, preserves empty roots/absent AMBR and applies shared IE policies with disjoint QFIs, backed by 380 independent complete-transfer cases. Correlation, conditional NAS forwarding and prescribed abnormal-condition responses remain caller-owned. Modify response/failure roots add 1,436 independent transfers with optional directional endpoints, unique/disjoint QFI results and response diagnostics; empty roots and absent/empty diagnostics stay distinct. Three Modify session lists and both complete procedure outcomes add 1,062 independent lists and 63 messages, bounded optional NAS/S-NSSAI, disjoint partial results and caller-owned transfer diagnostics. Explicit routing now distinguishes all 40 applicable outcomes, keeps 17 pending codec triggers disabled and supplies unsupported-procedure diagnostic headers only for absent procedures, with 2,321 independent routing envelopes and all Release 18 procedure metadata. See [`N3IWF-PROCEDURES.md`](../crates/opc-proto-ngap/N3IWF-PROCEDURES.md) for procedure-specific receive/error obligations and caller prerequisites. |
...
list counts 1–256, absent/empty roots and response header rejection. Public
response literals gain an explicit optional field; no new qualified outcome or
published fixture revision is claimed. Remaining optional fields, pending
procedure codecs and live interoperability evidence remain open under #787.
See [NGAP conformance](../crates/opc-proto-ngap/CONFORMANCE.md).
