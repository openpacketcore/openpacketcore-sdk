...
| IKEv2 codec | Experimental `opc-proto-ikev2` header/payload-chain scaffold, unknown payload preservation, protected-payload boundary metadata, and optional crypto-provider traits. Fragmentation framing checks remain future work before claiming RFC 7383 conformance. | IKE SA state machine, EAP-AKA procedure, cookie/retransmit policy, 3GPP profile enforcement, Child SA installation, and key-management policy. |
