...
| IKEv2 codec | Experimental `opc-proto-ikev2` header/payload-chain scaffold, unknown payload preservation, protected-payload boundary metadata, optional crypto-provider traits, IKE_AUTH cleartext helpers, Child SA negotiation intent, and RFC 7383 SKF structural helpers. | IKE SA state machine, EAP-AKA procedure, cookie/retransmit policy, 3GPP profile enforcement, Child SA lifecycle management, SKF decrypt/queue policy, and key-management policy. |
