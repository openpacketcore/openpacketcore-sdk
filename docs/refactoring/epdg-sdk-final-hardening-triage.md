...
| IKEv2 scaffold conformance defaults and fuzz workflow coverage | `T-3fc6bb76`; `.github/workflows/fuzz.yml` includes `opc-proto-ikev2` and `opc-proto-gtpv2c` | Converted and closed. This historical closeout predated the typed executable IKE-SA profiles and `SK`/`SKF` cryptographic providers now documented by the crate; product fragment queues and independent-peer fixture provenance remain outside that scaffold task. |
