...
| Readback/watch | Local history pages at most 64 records. Remote watch requests at most 16,384 bytes, responses at most 8,388,608 bytes, 32 concurrent connections and 256 client identities. A count-limited page can still exceed its byte ceiling. | `crates/opc-persist/src/types.rs`; `crates/opc-config-bus-consensus/src/remote_watch.rs` |
...
| Worst supported combination | 1,901,112 | Envelope plus non-envelope metadata at their independent maxima |
...
4. The eight existing accepted-proposal slots remain unchanged. An additional
   ...
   value-free definite rejection before submission; it cannot create an
   unbounded queue of large prepared commands. A sent proposal retains its
   reservation until its supervised completion, even after timeout/cancel.
...
unreadable. Opening a retained larger-profile store with a legacy-only profile
must refuse before mutation; the retained binding must prevent accidental
downgrade. That binding is an additional owned-symbol handoff prerequisite.
...
before exceeding its reservation. If that inventory cannot fit, the RFC must
...
and cancelled inbound traffic. Config admission cannot claim to bound memory
already allocated by an unrelated transport listener.
...
History retention is explicitly acknowledged; nothing here authorizes automatic
pruning of unresolved operations or audit evidence. Admission must account for
...
One at-limit record must read back locally and remotely. Count-limited history
pages must also obey byte limits: require an explicit smaller requested page
or a reviewed bounded-page contract, and demonstrate a one-record response
fits with worst metadata. Never silently omit committed records. Restore and
read paths distinguish historical compatibility from admission of new writes.
...
| Logical boundary | At 1,572,864 bytes succeeds; 1,572,865 rejects before provider/audit/consensus effects. Exercise escaping, multibyte strings and inconsistent size reporting. |
...
| Metadata/serialization | Aggregate metadata at/one-over 196,608; raw and tokenized path boundaries; principal/label/count bounds; actual command, worst singleton/forwarding and JSON log encodings. |
...
| Ambiguity/leader loss | Lose the original leader and lose the exact operation's response independently; recover the original operation read-only, preserving reported failure/indeterminate outcomes and avoiding a new mutation identity. |
...
| Concurrency/memory | Saturate all eight slots with at-limit inputs; observe exact byte reservations and high-water marks; one-over admission, cancellation, response loss and shutdown conserve ownership. Include transport and snapshot overlap. |
...
First review this contract and its source map. Governance requires a maintainer
approved RFC before implementation. Repository Discussions was disabled when
this draft was prepared; the maintainer must provide the discussion venue or
an explicit disposition of that prerequisite. This draft makes no approval
claim and does not enable a larger value.
...
override. Required-audit sequencing and authority custody remain with their
owners. Public exports and the final RFC number require a recorded handoff.
No transport, manifest or dependency expansion is proposed by this draft.

After approval: add shared plaintext admission, implement the config-only
sealed admission/resource contract, qualify every row, then integrate against
current main. Until then retain the existing 1 MiB command fence. Follow-up
