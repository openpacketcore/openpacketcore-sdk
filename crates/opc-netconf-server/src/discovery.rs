...
use opc_mgmt_schema::{DiscoveryMetadata, ModuleConformance, SchemaRegistry, SchemaSourceError};
...
pub fn render_yang_library(
    ...
    let root_selected = selection.contains("/yanglib:yang-library")
        || selection
            .schema_paths()
            .iter()
            .any(|p| p.starts_with("/yanglib:yang-library/"));
    if !root_selected {
    ...
    if selection.contains("/yanglib:yang-library/yanglib:content-id")
        || selection.contains("/yanglib:yang-library")
    {
        write_leaf(&mut writer, "content-id", registry.schema_digest())?;
    }

    let module_set_selected = selection.contains("/yanglib:yang-library/yanglib:module-set")
        || selection.contains("/yanglib:yang-library/yanglib:module-set/yanglib:name")
        || selection.contains("/yanglib:yang-library/yanglib:module-set/yanglib:module")
        || selection
            .contains("/yanglib:yang-library/yanglib:module-set/yanglib:import-only-module");
    if module_set_selected {
        ...
        write_leaf(&mut writer, "name", "complete")?;

        for d in meta {
            let tag = match d.conformance {
                ModuleConformance::Implement => "module",
                ModuleConformance::Import => "import-only-module",
            };
            writer
                .write_event(Event::Start(BytesStart::new(tag)))
                .map_err(|_| BindingError::projection("failed to write module start"))?;

            write_leaf(&mut writer, "name", d.name)?;
            if !d.revision.is_empty() {
                write_leaf(&mut writer, "revision", d.revision)?;
            }
            write_leaf(&mut writer, "namespace", served_namespace(registry, d))?;

            for imp in d.imports {
                writer
                    .write_event(Event::Start(BytesStart::new("import")))
                    .map_err(|_| BindingError::projection("failed to write import start"))?;
                write_leaf(&mut writer, "name", imp.name)?;
                if let Some(rev) = imp.revision {
                    write_leaf(&mut writer, "revision", rev)?;
                }
                writer
                    .write_event(Event::End(BytesEnd::new("import")))
                    .map_err(|_| BindingError::projection("failed to write import end"))?;
            }

            for feature in d.features {
                write_leaf(&mut writer, "feature", feature)?;
            }
            for deviation in d.deviations {
                write_leaf(&mut writer, "deviation", deviation)?;
            }
...
pub fn render_netconf_monitoring(
    ...
    let schemas_selected = selection.contains("/ncm:netconf-state/ncm:schemas")
        || selection.contains("/ncm:netconf-state/ncm:schemas/ncm:schema")
        || selection
            .schema_paths()
            .iter()
            .any(|p| p.starts_with("/ncm:netconf-state/ncm:schemas/ncm:schema/"));
    if !schemas_selected {
        return Ok(String::new());
    }
    ...
    for d in meta {
        ...
        write_leaf(&mut writer, "identifier", d.name)?;
        if !d.revision.is_empty() {
            write_leaf(&mut writer, "version", d.revision)?;
        }
        write_leaf(&mut writer, "format", "yang")?;
        write_leaf(&mut writer, "namespace", served_namespace(registry, d))?;
        write_leaf(&mut writer, "location", "NETCONF")?;
        writer
...
/// suitable for placement inside the `<get-schema>` `<data>` element and is
/// already XML-escaped by the caller's response writer.
...
fn served_namespace<'a>(registry: &'a dyn SchemaRegistry, d: &'a DiscoveryMetadata) -> &'a str {
    registry
        .served_models()
        .iter()
...
mod tests {
    ...
    fn yang_library_renders_content_id_from_schema_digest() {
        ...
        let xml = render_yang_library(&registry, ReadSelection::new(&["/yanglib:yang-library"]))
            .expect("render");
    ...
    fn yang_library_content_id_only_when_module_set_not_selected() {
        ...
        let xml = render_yang_library(
            &registry,
            ReadSelection::new(&["/yanglib:yang-library/yanglib:content-id"]),
        )
        .expect("render");
    ...
    #[test]
    fn yang_library_module_set_includes_imports_features_and_deviations() {
        let registry = test_registry();
        let xml = render_yang_library(
            &registry,
            ReadSelection::new(&["/yanglib:yang-library/yanglib:module-set"]),
        )
        .expect("render");
        assert!(xml.contains("<module-set>"));
    ...
    fn yang_library_import_only_module_uses_correct_tag() {
        ...
        let xml = render_yang_library(
            &registry,
            ReadSelection::new(&["/yanglib:yang-library/yanglib:module-set/yanglib:module"]),
        )
        .expect("render");
    ...
    #[test]
    fn yang_library_namespace_falls_back_to_empty_when_model_missing() {
        ...
        .expect("render");
        assert!(xml.contains("<namespace></namespace>"));
    }
    ...
    fn yang_library_fails_closed_without_discovery_metadata() {
        ...
        let err = render_yang_library(&registry, ReadSelection::new(&["/yanglib:yang-library"]))
