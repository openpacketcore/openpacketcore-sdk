...
//! unchanged survivor-election future, including panic or timeout unwinding. Samples are
...
impl ElectionTrace {
    ...
    fn finish_and_report(&self) {
        ...
        let Some(phase) = phase else {
            eprintln!("CONFIG_CAPACITY_RETENTION_ELECTION_TRACE phase_started=false");
            return;
        };
        eprintln!(
            "CONFIG_CAPACITY_RETENTION_ELECTION_TRACE phase_started=true samples={} deferred={} malformed={} overflow={}",
        ...
        for (index, sample) in phase.samples.into_iter().flatten().enumerate() {
            eprintln!(
                "CONFIG_CAPACITY_RETENTION_ELECTION_SAMPLE sample={index} phase_ns={} vote_age_ns={} timeout_ns={}",
