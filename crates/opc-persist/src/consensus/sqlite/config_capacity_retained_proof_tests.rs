...
async fn fixture_with_identity(
...
async fn assert_foreign_history_refused(profile: ConfigCapacityProfile, populated: bool) {
    let original = fixture(profile).await;
    let foreign = fixture_with_identity(
        profile,
        ConsensusIdentity::new(
            ConfigConsensusClusterId::from_bytes([0xD8; 32]),
            ConfigConsensusConfigurationId::from_bytes([0xD9; 32]),
            ConfigConsensusConfigurationEpoch::new(1).expect("same epoch"),
        ),
    )
    .await;
    if populated {
        initialize(&foreign).await;
        assert!(apply(
            &foreign,
            vec![entry(
                &foreign,
                1,
                append_intent(&foreign, 1, 128, 0, false, None),
            )],
        )
        .await[0]
            .result
    ...
    assert_eq!(
        foreign
            ...
            .load_latest()
            .await
            .expect("genuine foreign authority")
            .is_some(),
        populated,
    );

    let before_reads = {
        let source_shared = foreign.backend.conn();
        let source = source_shared.lock().await;
        let destination_shared = original.backend.conn();
        let destination = destination_shared.lock().await;
        let binding: Vec<u8> = destination
            .query_row("SELECT record FROM consensus_retained_binding", [], |row| {
                row.get(0)
            })
            .expect("original independent retained binding");
        let tx = destination
            .unchecked_transaction()
            .expect("replay transaction");
        // Copy genuine rows and MACs without issuing any new authentication.
        // The receiving authority remains open with its original admission and
        // retained binding. All copied tables keep their original schema.
        for table in [
            "config_history",
            "audit_trail",
            "config_lifecycle_audit",
            ...
            if table == "config_raft_capacity_records" && profile == ConfigCapacityProfile::Legacy {
                continue;
            }
            let columns = source
                .prepare(&format!("SELECT * FROM {table}"))
                .expect("fixed source table")
                .column_names()
                .join(", ");
            tx.execute(&format!("DELETE FROM {table}"), [])
                .expect("replace empty authority rows");
            copy_snapshot_table(
                &source,
                &tx,
                table,
            ...
            .expect("replay authentic foreign rows");
        }
        tx.commit().expect("complete replay");
        assert_eq!(
            destination
                .query_row("SELECT record FROM consensus_retained_binding", [], |row| {
                    row.get::<_, Vec<u8>>(0)
                },)
                .expect("retained binding after replay"),
            binding
        );
        assert_eq!(
            original.backend.config_consensus_identity(),
            Some(original.topology.identity())
        );
        super::super::history::validate_access_for_profile_sync(
            &destination,
            &original.key,
            true,
            Some(foreign.topology.identity()),
            profile,
            &SqliteWorkCancellation::new(),
        )
        .expect("copied history is authentic for the foreign authority using the same key");
        authority_digest(&destination)
    };

    fn assert_corrupt<T>(result: Result<T, crate::PersistError>, label: &str) {
        assert!(
            matches!(result, Err(error) if matches!(error.kind(), crate::PersistErrorKind::CorruptBlob)),
            "{label} must refuse foreign authority"
        );
    }
    assert_corrupt(original.backend.load_latest().await, "head");
    assert_corrupt(
        original.backend.load_committed_latest().await,
        "committed head",
    );
    assert_corrupt(
        original.backend.load_since(ConfigVersion::new(0), 1).await,
        "history",
    );
    assert_corrupt(
        original
            ...
            .await,
        "empty history lookup",
    );
    assert_corrupt(
        original.backend.retained_history_floor().await,
        "retention metadata",
    );
    assert_corrupt(
        original
            .backend
            .load_by_replay_lookup_digest(&"0".repeat(64))
            .await,
        "negative replay lookup",
    );
    for target in [
        crate::RollbackTarget::Previous,
        crate::RollbackTarget::ByTxId(tx_id(1)),
        crate::RollbackTarget::ByVersion(ConfigVersion::new(1)),
        crate::RollbackTarget::ByLabel("absent".to_owned()),
    ] {
        assert_corrupt(
            original.backend.load_rollback(target).await,
            "rollback lookup",
        );
    }
    let shared = original.backend.conn();
    let conn = shared.lock().await;
    assert_eq!(
        authority_digest(&conn),
        before_reads,
        "refused reads cannot modify durable authority"
    );
}

#[tokio::test]
async fn config_capacity_957_native_foreign_populated_history_refuses_live_reads() {
    assert_foreign_history_refused(PROFILE, true).await;
}

#[tokio::test]
async fn config_capacity_957_native_foreign_empty_history_refuses_live_negative_reads() {
    assert_foreign_history_refused(PROFILE, false).await;
}

#[tokio::test]
async fn config_capacity_957_native_foreign_legacy_history_refuses_live_negative_reads() {
    assert_foreign_history_refused(ConfigCapacityProfile::Legacy, false).await;
}
