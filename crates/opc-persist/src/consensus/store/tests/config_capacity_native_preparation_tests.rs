//! Native Durable singleton ownership control under the unchanged legacy
//! profile. Direct private pool issuance isolates reservation lifetime; this
//! does not enable BoundedV1 or qualify multi-node transport or memory peaks.
...
async fn config_capacity_957_native_accepted_preparation_survives_caller_cancellation() {
    ...
    let encrypted = opc_crypto::encrypt_reserved_bounded_config_envelope(
        pool.try_reserve().expect("eighth preparation"),
        &provider,
        &aad,
        plaintext,
    )
