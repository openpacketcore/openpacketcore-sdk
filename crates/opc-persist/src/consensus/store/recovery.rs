...
impl ConsensusConfigStore {
    ...
    pub fn prepare_recoverable_commit(
        ...
        // Preserve Legacy's authentication/error order. The bounded profile
        // must admit transferred capacities before allocating principal state.
        let legacy = self.capacity_profile() == opc_crypto::ConfigCapacityProfile::Legacy;
        if legacy
            && !crate::types::config_principal_matches_aad(
    ...
    /// missing, expired, or mismatched evidence remains `Unresolved`. Historical
    /// Legacy results without a result proof also remain `Unresolved`; the
    /// caller/intent handle alone never authenticates a stored result.
