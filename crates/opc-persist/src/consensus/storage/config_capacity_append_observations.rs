//! Opt-in test-only native append phase timings, without operation identities.
...
pub(super) fn queued(value: Observation) {
    record(value, "queued");
}
...
fn record(value: Observation, phase: &'static str) {
    if let Some((origin, entries)) = value {
        println!(
            "CONFIG_CAPACITY_NATIVE_APPEND phase={} entries={} elapsed_ms={}",
            phase,
            entries,
