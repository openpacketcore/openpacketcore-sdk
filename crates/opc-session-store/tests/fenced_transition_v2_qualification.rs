...
fn emit_bounded_scale_stall_observation(
    ...
    let achieved_ops_per_second = if elapsed.is_zero() {
        ...
        completed_operations as f64 / elapsed.as_secs_f64()
    };
    let latency_summary =
...
async fn sustained_131073_unique_v2_transitions_bind_exact_epoch_capacity() {
    ...
    let history =
        maintain_exact_history_batch(&stores, history_before_successor, &transient_retries, None)
            .await
                    ...
                    scheduled_at.push(
                        phase_started
                            + Duration::from_secs_f64(
                                scheduled_operations as f64 / target_rate as f64,
                            ),
                    );
...
async fn release_1010000_operation_successor_scale_is_bounded_and_recoverable() {
    ...
    // this a real process-lifecycle seam: no old RPC handler or engine can
    // serve the reopened paths.
    shutdown_fixed_cluster(&stores, &peer_slots).await;
    drop(stores);
    drop(peer_slots);
    ...
    eprintln!(
        ...
        FENCED_TRANSITION_V2_MAX_RETAINED_HISTORY_BYTES,
        transient_retries.load(Ordering::Relaxed),
        effect_snapshot.mutation_attempts,
        effect_snapshot.not_transmitted_retries,
        effect_snapshot.outcome_unknown_batches,
        effect_snapshot.status_attempts,
        effect_snapshot.status_retry_rounds,
