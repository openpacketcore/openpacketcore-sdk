...
#[path = "fenced_transition_v2_qualification/requests.rs"]
mod requests;
#[path = "fenced_transition_v2_qualification/resources.rs"]
mod resources;
use requests::*;
use resources::*;
...
fn private_lease_descriptor_rejects_a_fifo_and_a_shared_parent() {
    ...
    // Creation honors the caller's umask. Establish the actual shared mode
    // being rejected without changing process-wide filesystem policy.
    std::fs::set_permissions(&parent_path, std::fs::Permissions::from_mode(0o755))
        .expect("set the shared-parent negative-control mode");
