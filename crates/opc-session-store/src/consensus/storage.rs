...
mod tests {
    ...
    async fn wait_for_snapshot_staging_cleanup(directory: &Path) {
        for _ in 0..128 {
            ...
            if names.iter().all(|name| {
                !name.starts_with("incoming-")
                    ...
                    && !name.starts_with("seal-")
            }) {
                return;
            }
            tokio::task::yield_now().await;
        }
        panic!("deferred snapshot staging cleanup did not complete");
    }
    ...
    async fn snapshot_install_postcommit_detach_and_membership_failures_remain_committed() {
        ...
        wait_for_snapshot_staging_cleanup(&target_dir.path().join("snapshots")).await;
    }
