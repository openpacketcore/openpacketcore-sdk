...
pub enum SessionMutationIntent {
    ...
    ///
    /// The boxed payload keeps this newly appended capability from increasing
    /// the size of every existing in-process proposal envelope. Serde/Postcard
    /// serializes a `Box<T>` as `T`, so it does not alter the terminal
    /// variant's durable field sequence.
    #[doc(hidden)]
...
mod tests {
    ...
    fn roster_variants_append_after_existing_consensus_wire_tags() {
        ...
        for predecessor_bootstrap_membership_digest in [None, Some([0x83; 32])] {
            ...
            assert_eq!(
                encoded[0], 27,
                "recovery V2 keeps all historical intent discriminants stable"
            );
            assert_eq!(
