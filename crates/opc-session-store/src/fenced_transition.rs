...
pub enum AtomicFencedTransitionCapability {
    ...
    /// identified by [`fenced_transition_v2_profile_digest`], plus caller-side
    /// durable recovery of an exact protected request by its stable transition
    /// identity before any ambiguous dispatch.
    ///
