...
| Generic payload header and chain (`RFC 7296` §2.5, §3.2) | Experimental structural and receiver-profile coverage for unencrypted payloads | `src/payload.rs`; `tests/payload_chain.rs` walks a hand-authored SA -> Nonce chain, validates length fields, count limits, truncation, and byte-exact raw re-encode through `Message`. Network receive accepts and raw-preserves receiver-ignored reserved bits and the Critical bit on understood payloads; the sender-canonical profile diagnoses both, and unknown critical payloads still fail closed. |
...
- Cryptographic algorithms beyond the supported SA_INIT AES-GCM-16 and
  AES-CBC/SHA-2 `SK`/`SKF` profiles, deployment defaults (including default
