...
| New dedicated-bearer Child SA (`TS 24.302` R17 §7.2.7, §7.4.6.3; `RFC 7296` §1.3, §2.8) | Strict opened-payload boundary | `src/dedicated_bearer/exchange.rs` builds/decodes a non-rekey `CREATE_CHILD_SA` payload chain with SA, Nonce, optional KE, TSi, TSr, EPS QoS, TFT, and optional extended QoS/AMBR. It rejects `REKEY_SA`, duplicate or missing payloads, non-ESP/zero-or-wrong-sized SPIs, inconsistent DH/KE, non-create TFTs, ambiguous error/success sets, and malformed chains. Response correlation covers IKE SPIs, Message ID, exchange/flags, offered proposal/transform membership, KE group, and selector narrowing. Existing Child-SA key derivation consumes the decoded nonces. |
...
negative cardinality/shape/correlation cases, positive PFS request/response KE
correlation, rejection of mixed success/error response payloads, response
selector narrowing, Child-SA key-derivation compatibility, and byte identity
between a canonical TS 24.008 TFT value embedded in IKEv2 and the shared codec.
