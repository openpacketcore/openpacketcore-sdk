...
| IKE-SA crypto profile and KDF (`RFC 7296` §2.10, §2.13, §2.14, §2.17, §2.18; `RFC 2404`; `RFC 4868`; `RFC 6989`) | Typed algorithm and derivation coverage | `src/sa_init_crypto.rs` preserves PRF, DH, encryption/key size, and optional typed integrity in an immutable validated profile. It supports PRF-HMAC-SHA1/SHA2-256/384/512, AES-GCM-16 profile key material, and AES-CBC-128/192/256 with AUTH-HMAC-SHA1-96 or AUTH-HMAC-SHA2-256-128/384-192/512-256 key material. MODP-768/1024/2048 public values and shared secrets use exact 96/128/256-octet widths; peer public values must satisfy `1 < r < p-1`, with malformed width and invalid value reported distinctly. ECP-256/384/521 behavior remains unchanged. Transform selection is by type rather than wire order; duplicate, missing, unknown, and AEAD/integrity-contradictory sets fail closed. The SDK bounds nonce inputs to 16..=256 octets as a bounded-input policy; RFC 7296 section 2.10 supplies the 128-bit and half-PRF-key nonce requirements. Default KDF APIs enforce the selected PRF half-key floor. The explicit `Ikev2InitialExchangeNoncePolicy` admits only a 16-octet initiator nonce with PRF-HMAC-SHA2-512 through paired `derive_ike_sa_init_key_material_with_nonce_policy` and `derive_initial_child_sa_key_material` helpers; `derive_initial_child_sa_xfrm_keys` applies the same policy to the XFRM mapping. Those initial-Child helpers accept no new-DH input, so the opt-in cannot derive PFS or replacement Child SAs; generic `CREATE_CHILD_SA`, `derive_child_sa_xfrm_keys`, and IKE-SA rekey remain strict. The policy carries no nonce/key data, and errors/debug remain redaction-safe. Initial IKE-SA, Child-SA PFS, restore, PPK post-processing, and IKE-SA rekey derivation are covered; rekey uses the old PRF for `SKEYSEED`, the new PRF for the new seven-key expansion, and accepts only the selected group's fixed-width DH shared secret (DH1/2/14 96/128/256 octets; DH19/20/21 32/48/66 octets), reporting mismatches through the pre-existing redaction-safe invalid-key-length contract. RFC 2202/RFC 2404 SHA1 vectors, RFC 4231/RFC 4868 SHA2 vectors, independently reproduced MODP vectors, and synthetic initial/rekey/restore/Child round trips provide evidence. The SHA1 and MODP 1/2 algorithms are explicit compatibility only and are never inserted into caller policy. Protected-payload algorithm coverage is claimed separately below. |
...
| Pre-admission ECDSA key identity inspection | Bounded configuration-only boundary | `src/pre_admission.rs` accepts one exact unencrypted PKCS#8 DER value only after a conservative 4-KiB bound, inspects `AlgorithmIdentifier` before curve-specific parsing, and supports P-256/SHA-256 plus P-384/SHA-384. The RustCrypto secret-key object zeroizes on drop, while a separate explicitly zeroizing owner wipes the public-point derivation scalar; the result retains only a typed generation requirement and deterministic canonical public SPKI. A bounded exact-DER leaf-certificate helper compares the complete SPKI in constant time but performs no certificate trust validation. Empty, oversized, trailing, malformed, unsupported, and mismatch cases have stable payload-free errors. `tests/pre_admission_pkcs8.rs`, a compile-fail no-sign example, and counting-module evidence prove that this operation neither requires nor invokes the installed module and exposes no signing path. Actual key loading and signing remain exclusively module-routed (#413). |
...
| Child-SA rekey response (`RFC 7296` §1.3.3, §2.7-§2.10, §2.25, §3.3, §3.4, §3.9, §3.10, §3.13; `RFC 7383`) | Strict initiator opened-payload and once-only terminal boundary | `src/child_sa_rekey.rs` composes the existing `Ikev2CreateChildSaRekeyRequestBuild`, retains its canonical SA/TS offer, caller-supplied current Child-SA selector floor, initiator nonce, exact `REKEY_SA` protocol/SPI, old-IKE-SA SPI pair, Message ID, and endpoint role, and permits one correlated terminal response after authenticated `SK` open or `SKF` open/reassembly. Construction requires the request selector offer to cover the current SA and Ni to meet the established PRF's half-key nonce floor. Success requires an order-independent exact `SA, Nr, [KEr], TSi, TSr` singleton set, one executable selected ESP proposal from the retained offer with exactly one explicit SN/ESN selection, a matching valid PFS public value when selected, Nr meeting the PRF nonce floor, and `current selectors ⊆ response selectors ⊆ request offer`. Selector containment is an exact, bounded union check over complete protocol × port × IPv4/IPv6-address boxes: adjacent and overlapping partitions compose, one-address/one-port and cross-product gaps fail closed, wildcard protocol coverage is directional, canonical ANY includes OPAQUE, and OPAQUE never becomes an ordinary numeric range. Protocol-zero TS decode/build requires ports `0..65535`; malformed response selectors do not commit. The boundary returns both selected inbound ESP SPIs, the executable crypto profile, ESN/PFS selections, Nr/KEr, and selectors. Unknown critical and semantically prohibited known payloads fail closed. Unknown non-critical payloads and status Notifies follow RFC mandatory-ignore semantics: Preserve and normalized Reject retain them, while Drop discards them; Vendor IDs and ignorable extensions never change error terminality. Known response errors are type- and shape-checked, including exact `CHILD_SA_NOT_FOUND` SPI correlation and typed `INVALID_KE_PAYLOAD` group retention. Unrecognized error-range types remain terminal per §3.10.1 and retain bounded raw evidence behind redacted Debug; malformed or context-impossible known errors do not commit. Mixing an error with actual success payloads fails without committing, and valid success/error replay cannot commit twice. `tests/child_sa_rekey_response.rs` plus internal selector-set tests cover no-PFS/PFS success, SK/SKF header shape, payload order, exact header correlation, malformed/missing/duplicate payloads, invalid and unoffered proposal selections, mandatory SN/ESN shape, SPI/KE failures, nonce floors, union containment for split/overlapping IPv4 and IPv6 address/port ranges, wildcard/OPAQUE semantics, exact gap rejection, known/unknown/malformed peer errors, mandatory-ignore policy parity, mixed partial success, redacted diagnostics, and once-only replay. Retransmission/deadline/collision policy, SPI and key generation, KEYMAT invocation, installation, and old-SA retirement remain product-owned. |
...
| IKE_AUTH EAP-AKA packet projection (`RFC 4187`, `RFC 9048`, `RFC 5998`) | Opt-in strict structural coverage | `Ikev2EapPayload::project_aka` delegates complete Type 23/50 Request/Response packets to the canonical allocation-bounded `opc-proto-eap` parser. Generic EAP packets remain opaque. Cross-transport tests prove the same bytes and failures project identically through IKEv2 and SWm. No method MAC/AUTN/RES verification, key derivation, exchange state, or RFC 5998 safety/completion claim is made. |
...
| RFC 7427 signature-hash negotiation and IKE_AUTH signature AUTH (`RFC 7296` §2.15 method 1; `RFC 7427` §3-§4 method 14) | Bounded fail-closed typed mechanism coverage | `src/signature_hash.rs`, `src/ike_auth_signature.rs`, `tests/signature_hash_algorithms.rs`, `tests/signature_hash_admission.rs`, `tests/ike_auth_signature.rs`, `tests/local_auth_self_verification.rs`, and `tests/crypto_module_admission.rs` encode/decode `SIGNATURE_HASH_ALGORITHMS` (16431), retain exact ordered standardized/unassigned/private-use identifiers, enforce a 64-identifier resource bound, and reject reserved zero, malformed length/shape, empty, duplicate, omitted, unsupported-only, uncorrelated, and trailing states. The boundary decodes both complete correlated SA_INIT messages, validates and structurally retains each exact Nonce payload, and produces two independent sets as RFC 7427 requires: peer-offer ∩ admitted local signing support, and exact local offer ∩ admitted local verification support. No common hash between directions is required. Distinct non-copyable signing and verification authorities bind the applicable set to both exact SA_INIT messages, both nonce values, and the expected AUTH peer. Every method-14 operation consumes a one-operation authorization minted only after the caller presents that same request/response pair; a presented stale pair or nonce, opposite-message or opposite-direction nonce substitution, and wrong-direction use fail before transcript PRF or signature execution. The local signing authority additionally mints a sealed, non-copyable pre-transmit self-verification authorization that captures the exact local-role signed-octet inputs, selected hash, and local public credential without manufacturing an inverted peer role. It accepts method 14 only and consumes that authorization through the same admitted verification operation. Synthetic responder and initiator evidence covers ECDSA P-256/SHA-256, P-384/SHA-384, and feature-gated RSA; transcript, role, message, Nonce, identity, method, hash, key material, signature, and credential substitutions fail closed, while the ordinary peer authority rejects local-role octets. Counting-module evidence proves authority, Nonce, and method rejection invokes neither PRF nor signing/verification operations. Local offer construction and exact-wire negotiation both preflight the installed crypto module's immutable algorithm admission. The helpers sign and verify RFC 7296 signed octets with RSASSA-PKCS1-v1_5 SHA-256 (methods 1 and 14) and DER-encoded ECDSA P-256/P-384 (method 14) against a caller-supplied pinned SPKI or caller-trusted certificate SubjectPublicKeyInfo. SPKI and certificate constructors require one exact DER value and reject trailing bytes with stable redaction-safe errors. The product must retain each authority, exact exchange, and key material as one IKE-SA state because the SDK cannot infer an external application-session association from separately supplied byte slices and key material. No certificate-chain, validity-period, name, or key-usage validation is performed; the product owns certificate trust and transport/retransmission of the exact SA_INIT bytes. RSA signing is compiled only with the opt-in `rsa-signing` feature; RSA verification is always available. |
...
| New dedicated-bearer Child SA (`TS 24.302` R17 §7.2.7, §7.4.6.3; `RFC 7296` §1.3, §2.8, §3.3, §3.13; `RFC 4718` §4.4) | Strict opened-payload boundary | `src/dedicated_bearer/exchange.rs` builds/decodes a non-rekey `CREATE_CHILD_SA` payload chain with SA, Nonce, optional KE, TSi, TSr, EPS QoS, TFT, and optional extended QoS/AMBR. It rejects `REKEY_SA`, duplicate or missing payloads, non-ESP/zero-or-wrong-sized SPIs, PRF/unknown ESP transform types, exact duplicate transforms/attributes, unsupported algorithms, mixed AEAD/non-AEAD offers, inconsistent DH/KE, and create TFTs without an uplink-applicable filter. Same-type request alternatives remain valid, but every ESP offer contains at least one explicit SN/ESN transform. Selected responses contain exactly one ENCR and exactly one offered SN/ESN transform, optional single DH, and INTEG exactly when the selected supported encryption is non-AEAD; ID 0 explicitly selects no extended sequence numbers. Response correlation verifies the selected algorithms/attributes, optional `DH=NONE` semantics, KE group, IKE header, and exact selector-union narrowing. Shared TS build/decode requires protocol-zero selectors to use canonical ANY ports and accepts the distinct OPAQUE sentinel only for non-zero protocols. Existing Child-SA key derivation consumes the decoded nonces. |
...
| P-CSCF restoration `INFORMATIONAL` (`TS 23.380` §5.6.5.2; `TS 24.302` §7.2.3.2, §7.4.2.1; `RFC 7296` §3.2, §3.10.1, §3.15; `RFC 7651` §3-§4) | Strict opened-payload boundary | `src/pcscf_restoration.rs` owns configuration types 1/2 behind a typed, redaction-safe IPv4/IPv6 address API. The P-CSCF configuration-attribute types default to the `RFC 7651` §4 registered pair 20/21 and are caller-supplied via `Ikev2PcscfAttributeTypes`: that registration postdates deployed equipment, and `RFC 4306` §3.15.1 and the IANA registry reserve 16384-32767 for private use. The observation that peers negotiate P-CSCF there is this project's, taken from the wire; no RFC endorses it. Each family accepts only its own registered type or a private-use type (16384-32767, per the IANA registry and `RFC 4306` §3.15.1); every other code point is refused, because the registered space below names unrelated attributes such as `INTERNAL_IP4_ADDRESS` and `INTERNAL_IP4_DNS` and the unassigned range awaits expert-review allocation. That also forbids transposing the pair onto the other family's registered type. A pair is further rejected if either type sets the §3.15.1 reserved bit or if both families name the same type. The decoder recognizes the echo only on the supplied pair -- an attribute on any other type stays unsupported -- and reports the pair it matched on so a relaying node answers on the type it was asked on. Correlation requires the reply to have been read on the types the request was built on, compared only for the address families the exchange actually carried, so a reply byte-identical to a compliant registered exchange is not rejected on the strength of a type for a family that was never sent. A non-empty request accepts at most 128 entries, preserves every PGW-provided address and its exact order (including repeats), and emits each exact four- or sixteen-octet value in `CFG_REQUEST`; the bound is SDK resource policy, not a 3GPP cardinality claim. A single `CFG_REPLY` is accepted only when it carries one empty, unique, recognized attribute for every requested family, matching TS 24.302's distinct acknowledgement shape. Unsupported Configuration attributes, Vendor IDs, unfamiliar status Notify payloads, and unknown non-critical payloads are retained; unknown critical payloads, error-range Notify payloads, semantically invalid known payloads, and malformed extensions fail closed. Correlation requires that family set plus matching non-zero IKE SPIs, `INFORMATIONAL` exchange type, Message ID, response direction, and opposite Initiator flag. `tests/pcscf_restoration.rs` supplies literal valued same-family and dual-stack request bytes, repeat/order preservation, network-receiver reserved-field evidence, forward-compatible interleaved extensions, all family combinations, bounds/redaction checks, and negative absent/duplicate/valued-reply/wrong-type/truncated/critical/error/correlation cases. IKE SA protection, APCO interpretation, address selection, retransmission, and product session policy remain caller-owned. |
...
3. **Protected payload opening/sealing boundary:** use an identity-bound
   ...
   peer policy, choose retransmission behavior, run EAP-AKA method
   cryptography or session state, install Child SAs,
...
- IKE SA state machines, retransmission timers, exact-response caches, cookie policy, peer policy,
  ...
  evaluation, or message correlation outside the dedicated-bearer, Child-SA
  rekey, and P-CSCF restoration response validators.
- EAP-AKA method cryptography or session state, 3GPP ePDG profile enforcement,
...
`tests/child_sa_rekey_response.rs` uses only synthetic RFC 5737 documentation
address ranges and locally authored SPI, nonce, KE, proposal, and selector
values. Its successful and hostile response chains are derived from RFC 7296
§§1.3.3, 2.7-2.9, and 3.2-3.10. They contain no captured peer, subscriber,
address, identity, nonce, SPI, or key material.
...
AEAD/INTEG relationship validation, exact duplicate rejection, mandatory
explicit SN/ESN selection, same-type offer alternatives, positive PFS and DH-NONE request/response
